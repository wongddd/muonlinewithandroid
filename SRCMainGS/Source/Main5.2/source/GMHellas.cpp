...
int RenderHellasItemInfo(ITEM* ip, int textNum)
{
	...
	case ITEM_POTION + 28:
	{
		...
		TextNum++;
		sprintf(TextList[TextNum], GlobalText[1184]);
		...
		if (HeroLevel < g_iKalimaLevel[startIndex][0])
		{
			sprintf(TextList[TextNum], "\n");
			TextNum++;
			sprintf(TextList[TextNum], GlobalText[1123], g_iKalimaLevel[startIndex][0]);
...
{
	if (gMapManager->InHellas() == false) return
		false;

	PlayBuffer(SOUND_KALIMA_AMBIENT);

	if ((frame_scene_desplace % standlimit(40)) == 0)
	{
	...
	if (GetHellasLevel(Hero->Class, CharacterAttribute->Level) == KUNDUN_ZONE)
	{
		...
		if ((CurrX >= 25 && CurrY >= 44) && (CurrX <= 51 && CurrY <= 119) && (frame_scene_desplace % standlimit(50)) == 0)
		{
	...
	if ((rand() % 5) == 0)
	{
		...
		CreateParticleSync(BITMAP_LIGHT, Position, Hero->Object.Angle, Light, 7, 1.f, &Hero->Object);

		if ((rand() % 15) == 0)
		{
			vec3_t Angle = { 0.f, 0.f, 0.f };
			Position[2] = Hero->Object.Position[2] + 800.f;
			CreateEffectSync(9, Position, Angle, Light);
...
void CheckGrass(OBJECT* o)
{
	...
	if (Hero->Object.CurrentAction >= PLAYER_WALK_MALE && Hero->Object.CurrentAction <= PLAYER_RUN_RIDE_WEAPON
		...
		|| Hero->Object.CurrentAction >= PLAYER_FENRIR_WALK && Hero->Object.CurrentAction <= PLAYER_FENRIR_WALK_WEAPON_LEFT
...
bool RenderHellasVisual(OBJECT* o, BMD* b)
{
	...
	case 35:
		...
		CreateParticleSync(BITMAP_LIGHT, o->Position, o->Angle, Light, 6, 1.f, o);
		o->HiddenMesh = -2;
		break;
	case 36:
		Vector(1.f, 1.f, 1.f, Light);
		CreateParticleSync(BITMAP_TRUE_BLUE, o->Position, o->Angle, Light, 0);
		o->Scale = 0.5f;
		o->HiddenMesh = -2;
		break;
	case 37:
		Vector(1.f, 1.f, 1.f, Light);
		CreateParticleSync(BITMAP_WATERFALL_5, o->Position, o->Angle, Light, 0);
		o->Scale = 0.5f;
	...
	case 38:
		...
		if (rand() % 2 == 0)
		{
			CreateParticleSync(BITMAP_WATERFALL_1, o->Position, o->Angle, Light, 0);
		}
		PlayBuffer(SOUND_KALIMA_WATER_FALL);
		o->Scale = 0.5f;
		break;
	case 39:
		Vector(1.f, 1.f, 1.f, Light);
		CreateParticleSync(BITMAP_WATERFALL_3 + (rand() % 2), o->Position, o->Angle, Light, 0);
		o->Scale = 0.5f;
		break;
	case 40:
		Vector(1.f, 1.f, 1.f, Light);
		if (rand() % 4 == 0)
		{
			CreateParticleSync(BITMAP_WATERFALL_2, o->Position, o->Angle, Light, 0);
		}
		o->Scale = 0.5f;
		break;
	}
...
bool RenderHellasObjectMesh(OBJECT* o, BMD* b)
{
	...
	else if (o->Type == MODEL_CUNDUN_GHOST)
	{
		...
		b->RenderBody(RENDER_TEXTURE | RENDER_DARK, o->Alpha, o->BlendMesh, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV, o->HiddenMesh);
		Vector(0.0f, 0.0f, 0.0f, b->BodyLight);
		RenderPartObjectEdge(o->Type, b, o, RENDER_COLOR, true, 0.7f);
...

		{
			...
			CreateParticleSync(BITMAP_SMOKE, Position, o->Angle, Light, 17, 3.0f);
		}
		return true;
	}
...
int CreateBigMon(OBJECT* o)
{
	...
	float ajust_fps_render = frame_scene_desplace;

	ajust_fps_render *= (REFERENCE_FPS / (double)gmProtect->ajust_fps_render);

	if ((static_cast<int>(ajust_fps_render) % 100) == 0)
	{
		o->Live = true;
		OpenMonsterModel(33);
		...
		Vector(Hero->Object.Position[0] - 1000 - rand() % 200, Hero->Object.Position[1] - 500 + rand() % 200, Hero->Object.Position[2] - 800.f, o->Position);
	}
	else
	{
		o->Live = false;
...
void MoveBigMon(OBJECT* o)
{
	...
	if (o->LifeTime < 20)
	{
		o->Alpha /= timepow(1.2f);
		o->Velocity += timefac(0.5f);
		o->Angle[0] += timefac(2.0f);
	}
...
void CreateMonsterSkill_ReduceDef(OBJECT* o, double AttackTime, BYTE time, float Height)
{
	if (AttackTime >= time)
	{
...
void CreateMonsterSkill_Poison(OBJECT* o, double AttackTime, BYTE time)
{
	if (AttackTime >= time)
	{
		float  Matrix[3][4];
		...
		for (int i = 0; i < 5; i++)
		{
			...
			CreateEffectSync(MODEL_FIRE, Position, o->Angle, Light, 8, NULL, 0);
		}
...
void CreateMonsterSkill_Summon(OBJECT* o, double AttackTime, BYTE time)
{
	if (AttackTime >= time)
	{
		CreateEffectSync(MODEL_CIRCLE, o->Position, o->Angle, o->Light, 3, o);
		CreateEffectSync(MODEL_CIRCLE_LIGHT, o->Position, o->Angle, o->Light, 4);
	}
}
...
CHARACTER* CreateHellasMonster(int Type, int PositionX, int PositionY, int Key)
{
	...
	case 275:
		...
		c->Weapon[1].Level = 0;
		c->Object.Scale = 2.0f;o = &c->Object;
		strcpy(c->ID, "��¥���");
		o->LifeTime = 100;
		break;
	}
...
bool AttackEffect_HellasMonster(CHARACTER* c, CHARACTER* tc, OBJECT* o, OBJECT* to, BMD* b)
{
	...
	case 277:
		...
		case Skill_Fire_Slash_Mastery:
		case Skill_Fire_Slash:
			CreateMonsterSkill_ReduceDef(o, (double)c->AttackTime, 13, 200.f);
			break;

		case AT_SKILL_POISON:
		case Skill_Poison_Strengthener:
			CreateMonsterSkill_Poison(o, (double)c->AttackTime, 13);
			break;

		case AT_SKILL_MONSTER_SUMMON:
			CreateMonsterSkill_Summon(o, (double)c->AttackTime, 13);
			break;
	...
	case 274:
		...
		{
			CreateEffectSync(MODEL_SKILL_INFERNO, o->Position, o->Angle, o->Light, 5, o);
			CreateEffectSync(BITMAP_FLAME, o->Position, o->Angle, o->Light, 2, o);
			c->AttackTime = 15;
		}
		return true;
...
bool MoveHellasMonsterVisual(OBJECT* o, BMD* b)
{
	...
	case MODEL_WARCRAFT:
		if (o->CurrentAction == 0)
		{
			...
			CreateParticleSync(BITMAP_SMOKE + 1, Position, o->Angle, o->Light);
			CreateParticleSync(BITMAP_SMOKE + 1, Position, o->Angle, o->Light);
			CreateEffectSync(MODEL_STONE1, o->Position, o->Angle, o->Light);
			CreateEffectSync(MODEL_STONE2, o->Position, o->Angle, o->Light);
		}
		o->BlendMesh = 1;
	...
	case MODEL_MONSTER01 + 64:
		if (rand() % 2 == 0)
		{
			...
			Vector(1.0f, 0.2f, 0.0f, Light);
			CreateParticleSync(BITMAP_SMOKE, Position, o->Angle, Light, 17);
		}
		return true;
...
bool RenderHellasMonsterVisual(CHARACTER* c, OBJECT* o, BMD* b)
{
	...
	case MODEL_MONSTER01 + 63:
		...
		if (o->CurrentAction != MONSTER01_DIE)
		{
			...
			for (i = 0; i < 5; ++i)
			{
				...
				if ((rand() % 2) == 0)
				{
					CreateParticleSync(BITMAP_BUBBLE, Position, o->Angle, Light, 1);
				}
	...
	case MODEL_MONSTER01 + 64:
		...
		else
		{
			...
			Vector(0, 0, 0, p);
			{
				...
				if ((rand() % 2) == 0)
				{
					CreateParticleSync(BITMAP_SMOKE, Position, o->Angle, Light, 13, fRoar);
				}
			}
		...
		if (o->CurrentAction != MONSTER01_DIE)
		{
			...
			if (o->CurrentAction == MONSTER01_ATTACK1)
			{
				for (i = 10; i < 16; ++i)
				{
					...
					CreateJointSync(BITMAP_JOINT_THUNDER, pos1, pos2, o->Angle, 7, NULL, 30.f);
				}
				for (i = 31; i < 37; ++i)
				{
					b->TransformPosition(o->BoneTransform[i], p, pos1, true);
	...
	case MODEL_MONSTER01 + 67:
		if (o->CurrentAction != MONSTER01_DIE)
		{
			...
			if (c->MonsterIndex == 145)
			{
				...
				if ((rand() % 2) == 0)
				{
					CreateParticleSync(BITMAP_SMOKE, Position, o->Angle, Light, 13);
				}
			}
			else
			{
	...
	case MODEL_MONSTER01 + 68:
		if (o->CurrentAction != MONSTER01_DIE)
		{
			...
			CreateSprite(BITMAP_ENERGY, pos, 0.5f + (Luminosity * 0.2f), Light, o, -WorldTime * 0.1f);
			CreateParticleSync(BITMAP_LIGHT, pos, o->Angle, Light, 0, 1.1f);
			...
			for (i = 0; i < 5; ++i)
			{
				...
				if ((o->CurrentAction == MONSTER01_STOP1 || o->CurrentAction == MONSTER01_STOP2) && (rand() % 50) == 0)
				{
					...
					CreateJointSync(BITMAP_FLARE + 1, Position, pos, Angle, 5, NULL, 20.f);
				}
			}
		}
		return true;
	...
	case MODEL_MONSTER01 + 69:
		if (o->CurrentAction != MONSTER01_DIE)
		{
			...
			for (i = 0; i < 6; ++i)
			{
				...
				if (o->SubType == 9)
				{
					CreateParticleSync(BITMAP_FIRE + 1, Pos1, o->Angle, Light, 1, 7.2f / (i / 2 + 6));
					CreateParticleSync(BITMAP_FIRE + 1, Pos2, o->Angle, Light, 1, 7.2f / (i / 2 + 6));
				}
				else
				{
					CreateParticleSync(BITMAP_FIRE + 3, Pos1, o->Angle, Light, 12, 7.2f / (i / 2 + 6) * 0.5f);
					CreateParticleSync(BITMAP_FIRE + 3, Pos2, o->Angle, Light, 12, 7.2f / (i / 2 + 6) * 0.5f);
				}
			}
		}
		else
...
bool RenderHellasMonsterObjectMesh(OBJECT* o, BMD* b)
{
	...
	else if (o->Type == MODEL_MONSTER01 + 64)
	{
		if (o->CurrentAction == MONSTER01_DIE && o->AnimationFrame > 14.8f && o->LifeTime == 90)
		{
			...
			b->TransformPosition(o->BoneTransform[4], p, Position, true);
			CreateEffectSync(MODEL_CUNDUN_PART8, Position, o->Angle, o->Light, 3, o, -130, 3);
			Vector(24.0f, -7.5f, 32.5f, p);
			b->TransformPosition(o->BoneTransform[4], p, Position, true);
			CreateEffectSync(MODEL_CUNDUN_PART1, Position, o->Angle, o->Light, 3, o, -130, 4);
			Vector(24.0f, -8.5f, -32.5f, p);
			b->TransformPosition(o->BoneTransform[4], p, Position, true);
			CreateEffectSync(MODEL_CUNDUN_PART2, Position, o->Angle, o->Light, 3, o, -130, 5);
			Vector(-0.5f, 4.0f, 0.5f, p);
			b->TransformPosition(o->BoneTransform[4], p, Position, true);
			CreateEffectSync(MODEL_CUNDUN_PART3, Position, o->Angle, o->Light, 2, o, -130, 6);
			Vector(-2.5f, -22.0f, 54.0f, p);
			b->TransformPosition(o->BoneTransform[4], p, Position, true);
			CreateEffectSync(MODEL_CUNDUN_PART4, Position, o->Angle, o->Light, 3, o, -130, 1);
			Vector(-4.5f, -24.5f, -53, p);
			b->TransformPosition(o->BoneTransform[4], p, Position, true);
			CreateEffectSync(MODEL_CUNDUN_PART5, Position, o->Angle, o->Light, 3, o, -130, 2);
			Vector(-136.0f, -153.5f, 0, p);
			b->TransformPosition(o->BoneTransform[4], p, Position, true);
			CreateEffectSync(MODEL_CUNDUN_PART6, Position, o->Angle, o->Light, 4, o, -10, 2);
			Vector(-135.0f, -153.0f, 0.0f, p);
			b->TransformPosition(o->BoneTransform[4], p, Position, true);
			CreateEffectSync(MODEL_CUNDUN_PART7, Position, o->Angle, o->Light, 5, o, -130, 2);

			CreateEffectSync(MODEL_CUNDUN_SKILL, o->Position, o->Angle, o->Light, 2);
		}
		else
		{
			if (o->CurrentAction == MONSTER01_DIE && o->AnimationFrame >= 8 && o->LifeTime >= 100)
			{
				...
				for (int i = 0; i < iCount; ++i)
				{
					...
					VectorCopy(o->Position, Position);
					Position[2] += 200.f;
					CreateJointSync(BITMAP_JOINT_SPIRIT, Position, Position, Angle, 3, NULL, 50.f, 0, 0);
					CreateJointSync(BITMAP_JOINT_SPIRIT2, Position, Position, Angle, 3, NULL, 50.f, 0, 0);
				}
				EarthQuake = (float)(rand() % 8 - 8) * 0.1f;
			}
	...
	if (success)
	{
		if (g_isCharacterBuff(o, eBuff_PhysDefense) || g_isCharacterBuff(o, eBuff_Defense))
		{
			...
			RenderPartObjectEdge(o->Type, b, o, RENDER_CHROME | RENDER_BRIGHT, true, 1.3f);
		}
	}
