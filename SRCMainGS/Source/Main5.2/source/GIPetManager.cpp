...
namespace giPetManager
{
	...
	bool RequestPetInfo(unsigned __int16 sx, unsigned __int16 sy, ITEM* pItem)
	{
			...
			|| g_RenderItemInfoBackup.Level != pItem->Level)
		{
			...
			int iInvenType = 0;
				int iSlotIndex = g_pMyInventory->GetPointedItemIndex();
...
if (iSlotIndex != -1)
	...
	bool RenderPetItemInfo(int sx, int sy, ITEM* pItem, int iInvenType)
	{
		...
		sprintf(TextList[TextNum++], "\n"); SkipNum++;
		sprintf(TextList[TextNum++], "\n"); SkipNum++;
		sprintf(TextList[TextNum++], GlobalText[201], pPetInfo->m_dwExp1, pPetInfo->m_dwExp2); SkipNum++;
		sprintf(TextList[TextNum++], "%s : %d", GlobalText[368], pPetInfo->m_wLevel); SkipNum++;
