...
CSQuest::CSQuest(void) : m_byClass( 255 ), m_byCurrQuestIndex( 0 ), m_byCurrQuestIndexWnd( 0 ),
						m_byStartQuestList( 0 ), m_shCurrPage( 0 ), m_byViewQuest( QUEST_VIEW_NONE ), m_byQuestType( TYPE_QUEST ), m_iStartX(gwinhandle->GetScreenX() -190), m_iStartY(0)
{
...
{
	ITEM* pItem = NULL;
	int nFindItemCount = 0;int nItemsInInven = g_pMyInventory->GetNumberOfItems();

	for (int i = 0; i < nItemsInInven; ++i)
	{
		pItem = g_pMyInventory->GetItem(i);if (pItem && nType == pItem->Type)
		{
