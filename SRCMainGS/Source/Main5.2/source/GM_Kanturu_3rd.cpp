...
{
	return (gMapManager->currentMap == WD_39KANTURU_3RD) ? true : false;
}
...
void M39Kanturu3rd::CheckSuccessBattle(BYTE State, BYTE DetailState)
{
	...
	char FileName[64];
	char WorldName[32];
	sprintf(WorldName, "World%d", gMapManager->currentMap + 1);

	//
	if (KanturuSuccessMap)
	{
		PlayBuffer(SOUND_KANTURU_3RD_MAP_SOUND02);
		sprintf(FileName, "Data\\%s\\EncTerrain%d.att", WorldName, (gMapManager->currentMap + 1) * 10 + 1);
	}
	else
	{
		sprintf(FileName, "Data\\%s\\EncTerrain%d.att", WorldName, gMapManager->currentMap + 1);
	}
...
bool M39Kanturu3rd::RenderKanturu3rdObjectVisual(OBJECT* o, BMD* b)
{
	...
	case 0:
	{
		...
		else
		{
			...
			if (o->AnimationFrame >= 5.0f && o->AnimationFrame < 12.5f)
				CreateParticleSync(BITMAP_SMOKE, Pos, o->Angle, Light, 43, 1.5f);
		}
	...
	case 32:
	{
		if (!(rand() % 3))
		{
			...
			CreateParticleSync(BITMAP_TWINTAIL_WATER, o->Position, o->Angle, Light, 1);
		}
	}
	break;
	case 46:
	...
	case 74:
	{
		...
		{
			if (rand() % 5 == 0)
				CreateJointSync(BITMAP_JOINT_THUNDER + 1, o->Position, o->Position, o->Angle, 11, NULL, o->Scale * 15.0f);
		}
	}
	break;
	case 47:
	...
	case 48:
	{
		if (!(rand() % 3))
		{
			CreateParticleSync(BITMAP_CLOUD, o->Position, o->Angle, o->Light, 10, o->Scale, o);
		}
	}
	break;
	case 49:
	...
	case 52:
	{
		if (!(rand() % 3))
		{
			...
			CreateParticleSync(BITMAP_TRUE_BLUE, o->Position, o->Angle, o->Light, 1, o->Scale);
		}
	}
	break;
	case 53:
	{
		if (!(rand() % 3))
		{
			...
			CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 46, o->Scale);
		}
	}
	break;
	case 54:
	{
		...
		{
			o->HiddenMesh = -1;
			CreateJointSync(BITMAP_JOINT_THUNDER, o->Position, o->Position, o->Angle, 19, NULL, o->Scale * 10.0f);
		}
	}
	break;
	}
...
bool M39Kanturu3rd::RenderKanturu3rdObjectMesh(OBJECT* o, BMD* b, bool ExtraMon)
{
	if (IsInKanturu3rd())
	{
		...
		case 0: // ����
		{
			if (g_Direction.m_CKanturu.GetMayaExplotion())
			{
				o->Light[0] /= timepow(1.02f);
				o->Light[1] /= timepow(1.02f);
				o->Light[2] /= timepow(1.02f);
			...
			if (g_Direction.m_CKanturu.GetMayaExplotion())
			{
				o->StartPosition[0] /= timepow(1.02f);
				o->StartPosition[1] /= timepow(1.02f);
				o->StartPosition[2] /= timepow(1.02f);
...
bool M39Kanturu3rd::MoveKanturu3rdMonsterVisual(OBJECT* o, BMD* b)
{
	...
	case MODEL_MONSTER01 + 121:
	{
		...
		else if (o->CurrentAction == MONSTER01_ATTACK2 && (o->AnimationFrame >= 5.2f && o->AnimationFrame <= 5.8f))
		{
			...
			CreateEffectSync(MODEL_STORM2, Position, o->Angle, Light, 0);
			CreateEffectSync(BITMAP_BOSS_LASER, Position, o->Angle, Light, 2);
		}
		else if (o->CurrentAction == MONSTER01_DIE && o->AnimationFrame >= 3.0f)
		{
			vec3_t Position;
			BoneManager::GetBonePosition(o, "Body_Bone13", Position);
			CreateParticleSync(BITMAP_SMOKE + 3, Position, o->Angle, Light, 3, 1.5f);
			CreateParticleSync(BITMAP_SMOKE + 3, Position, o->Angle, Light, 3, 1.5f);

			vec3_t Angle;
			Vector((float)(rand() % 100) - 50.0f, (float)(rand() % 100) - 50.0f, (float)(rand() % 360), Angle);
			float Scale = (float)(rand() % 40) + 30.0f;
			CreateJointSync(BITMAP_JOINT_SPIRIT2, Position, Position, Angle, 8, NULL, Scale, 0, 0);
		}
	...
	case MODEL_MONSTER01 + 118:
	{
		...
		if (o->CurrentAction == MONSTER01_DIE && g_Direction.m_CKanturu.m_iMayaState < KANTURU_MAYA_DIRECTION_MAYA3)
		{
			...
			for (int i = 0; i < 5; i++)
			{
				...
				CreateParticleSync(BITMAP_WATERFALL_3, Pos, o->Angle, Light, 6, 2.f, o);
			}
		}
		else
			o->BlendMesh = -1;
		...
		if (o->CurrentAction == MONSTER01_STOP2 && g_Direction.m_CKanturu.m_iMayaState >= KANTURU_MAYA_DIRECTION_MAYA3)
		{
			...
			CreateParticleSync(BITMAP_SMOKE + 3, Pos, o->Angle, Light, 3, 1.5f);
			CreateParticleSync(BITMAP_SMOKE + 3, Pos, o->Angle, Light, 4, 0.8f);
		}
...
bool M39Kanturu3rd::RenderKanturu3rdMonsterVisual(CHARACTER* c, OBJECT* o, BMD* b)
{
	...
	case MODEL_MONSTER01 + 121:
	{
		...
		BoneManager::GetBonePosition(o, "Body_Bone13", Position);
		CreateParticleSync(BITMAP_FIRE + 1, Position, o->Angle, Light, 3, 1.7f);
		...
		if (rand() % 2 == 0)
			CreateParticleSync(BITMAP_SPARK + 1, Position, o->Angle, Light, 16, 1.0f);
	...
	case MODEL_SMELTING_NPC:
	{
		...
		if (rand() % 10 == 0)
		{
			...
			CreateParticleSync(BITMAP_SPARK + 1, Position, o->Angle, Light, 17, 0.6f);
		}
	}
	return true;
	}
...
bool M39Kanturu3rd::AttackEffectKanturu3rdMonster(CHARACTER* c, OBJECT* o, BMD* b)
{
	...
	case 361:
	{
		if (o->CurrentAction == MONSTER01_ATTACK3 && c->AttackTime >= 14)
		{
			...
			CreateEffectSync(MODEL_SUMMON, o->Position, o->Angle, Light, 3);
		}
		...
		if (g_Direction.m_CKanturu.GetMayaExplotion())
		{
			...
			if (iMayaDie_Counter == 0)
			{
				if (rand() % 5 == 0)
				{
					for (int j = 0; j < 3; j++)
					{
						...
						CreateParticleSync(BITMAP_EXPLOTION, Pos, o->Angle, Light, 1, (1.5f + rand() % 10 / 20.0f));
					}
				}
			...
			else
			{
				for (int i = 0; i < 3; i++)
				{
					...
					CreateParticleSync(BITMAP_EXPLOTION, Pos, o->Angle, Light, 1, 1.0f + (i * 1.5f) + rand() % 10 / 10.0f);

					Vector(1.0f, 0.5f, 0.3f, Light);
					Vector(rand() % 200 - 100.0f, rand() % 200 - 100.0f, rand() % 200 - 100.0f, Position);
					b->TransformPosition(BoneTransform[Index[rand() % 11]], Position, Pos, false);
					CreateParticleSync(BITMAP_SMOKE, Pos, o->Angle, Light, 47, 1.5f + rand() % 4);
				}
...
void M39Kanturu3rd::Kanturu3rdSuccess()
{
	...
	fWidth = 372.0f;
	fHeight = 99.0f;
	fPosX = (gwinhandle->GetScreenX() - fWidth) / 2.0f;
	fPosY = (gwinhandle->GetScreenY() - fHeight) / 2.0f;
	tu = fWidth / 512.f;
	tv = fHeight / 128.f;

	if (fAlpha <= 1.0f)
		fAlpha += timefac(0.01f);
...
void M39Kanturu3rd::ChangeBackGroundMusic(int iWorld)
{
