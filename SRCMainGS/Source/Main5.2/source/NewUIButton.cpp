...
namespace
{
	void PointSet(FPOINT& p, int x, int y)
	{
		p.x = x; p.y = y;
	}

	void RenderText(const char* text, int x, int y, int sx, int sy, int hfont, DWORD color, DWORD backcolor, int sort)
	{
...
void CNewUIBaseButton::SetPos(float x, float y)
{
	PointSet(m_Pos, x, y);
}

void CNewUIBaseButton::SetSize(float sx, float sy)
{
...
void SEASON3B::CNewUIButton::ChangeButtonImgState(bool imgregister, int imgindex, bool overflg, bool isimgwidth, bool bClickEffect, float su, float sv, float uw, float vh)
{
	m_bClickEffect = bClickEffect;

#if MAIN_UPDATE > 603
	if (imgindex == CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY)
	{
		uw = 411.f; vh = 96.f;
		imgindex = BITMAP_msgbox_btn_empty;
	}
	else if (imgindex == CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY_BIG)
	{
		uw = 512.f; vh = 96.f;
		imgindex = BITMAP_msgbox_btn_empty_big;
	}
	else if (imgindex == CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY_SMALL)
	{
		uw = 304.f; vh = 96.f;
		imgindex = BITMAP_msgbox_btn_empty_small;
	}
	else if (imgindex == CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY_VERY_SMALL)
	{
		uw = 304.f; vh = 96.f;
		imgindex = BITMAP_msgbox_btn_empty_very_small;
	}
#endif
	...
	if (imgregister)
	{
		if (overflg)
		{
			if (isimgwidth)
			{
				this->RegisterButtonState(BUTTON_STATE_UP, imgindex, 0, su, sv, uw, vh);
				this->RegisterButtonState(BUTTON_STATE_OVER, imgindex, 1, su + uw, sv, uw, vh);
				this->RegisterButtonState(BUTTON_STATE_DOWN, imgindex, 2, su + uw + uw, sv, uw, vh);
			}
			else
			{
				this->RegisterButtonState(BUTTON_STATE_UP, imgindex, 0, su, sv, uw, vh);
				this->RegisterButtonState(BUTTON_STATE_OVER, imgindex, 1, su, sv + vh, uw, vh);
				this->RegisterButtonState(BUTTON_STATE_DOWN, imgindex, 2, su, sv + vh + vh, uw, vh);
			}
		}
		else
		{
			...
			{
				this->RegisterButtonState(BUTTON_STATE_UP, imgindex, 0, su, sv, uw, vh);
				this->RegisterButtonState(BUTTON_STATE_DOWN, imgindex, 1, su + uw, sv, uw, vh);
			}
			else
			{
				this->RegisterButtonState(BUTTON_STATE_UP, imgindex, 0, su, sv, uw, vh);
				this->RegisterButtonState(BUTTON_STATE_DOWN, imgindex, 1, su, sv + vh, uw, vh);
			}
		}
		this->ChangeFrame(BUTTON_STATE_UP);
		this->ChangeImgWidth(isimgwidth);
	}
}


void SEASON3B::CNewUIButton::ChangeButtonInfo(float x, float y, float sx, float sy)
{
	SetPos(x, y);
	SetSize(sx, sy);
}
...
void SEASON3B::CNewUIButton::ChangeImgIndex(int imgindex, int curimgstate, float su, float sv, float uw, float vh)
{
...
void SEASON3B::CNewUIButton::ChangeImgDrawCoord(BUTTON_STATE eventstate, float su, float sv, float uw, float vh)
{
	if (m_ButtonInfo.size() != 0)
	{
		ButtonStateMap::iterator iter = m_ButtonInfo.find(static_cast<int>(eventstate));
...
void SEASON3B::CNewUIButton::ChangeFrame(BUTTON_STATE btn_state)
{
	if (btn_state != BUTTON_STATE_NONE)
		this->m_EventState = (btn_state);
...
void SEASON3B::CNewUIButton::updatefont(int hfont)
{
	if (hfont == 1)
	{
...
bool SEASON3B::CNewUIButton::Render(bool RendOption)
{
	if (m_ButtonInfo.size() != 0)
	{
		if (m_BtnInfo != NULL && m_BtnInfo->uw != 0.f && m_BtnInfo->vh != 0.f)
		{
			ButtonInfo* btn = m_BtnInfo;
			SEASON3B::RenderImageF(btn->s_ImgIndex, m_Pos.x, m_Pos.y, m_Size.x, m_Size.y, btn->s_imgColor, btn->su, btn->sv, btn->uw, btn->vh);
		}
		else
		{
			if (RendOption == true)
			{
				RenderImage(m_CurImgIndex, m_Pos.x, m_Pos.y, m_Size.x, m_Size.y, 0.f, m_CurImgState * m_Size.y, 0.5625, 0.453125);
			}
			else
			{
	...
	if (m_Name.size() != 0)
	{
		...
		updatefont(m_hTextFont);

		if (m_bClickEffect && (GetBTState() == BUTTON_STATE_DOWN))
		{
			RenderText(m_Name.c_str(), x + m_iMoveTextPosX + 1, y + m_iMoveTextPosY + 1, m_Size.x, 0, m_hTextFont, m_NameColor, m_NameBackColor, RT3_WRITE_CENTER_TO_TOP);
		}
		else
		{
			RenderText(m_Name.c_str(), x + m_iMoveTextPosX, y + m_iMoveTextPosY, m_Size.x, 0, m_hTextFont, m_NameColor, m_NameBackColor, RT3_WRITE_CENTER_TO_TOP);
		}
	}
	...
	if (m_TooltipText.size() != 0)
	{
		if (CheckMouseIn(m_Pos.x, m_Pos.y, m_Size.x, m_Size.y))
		{
			...
			int x = m_Pos.x + ((m_Size.x - Fontsize.cx) / 2);
			int y = m_Pos.y + m_Size.y + 2;

			int _iTempWidth = x + Fontsize.cx + 6;
			x = (_iTempWidth > GetWindowsX) ? (x - (_iTempWidth - GetWindowsX)) : x;

			if (m_IsTopPos)
				y = m_Pos.y - (Fontsize.cy + 2);

			x += m_iMoveTextTipPosX;
			y += m_iMoveTextTipPosY;
...
void SEASON3B::CNewUIButton::CurSize(WORD uw, WORD uv)
{
	for (ButtonStateMap::iterator pair = m_ButtonInfo.begin(); pair != m_ButtonInfo.end(); pair++)
	{
		...
		btinfo->su = 0.0;
		btinfo->sv = 0.0 + (uv * btinfo->s_BTstate);
		btinfo->uw = uw;
		btinfo->vh = uv;
	}
	//m_uw = uw;
...
void CNewUIRadioButton::updatefont(int hfont)
{
	if (hfont == 1)
	{
		g_pRenderText->SetFont(g_hFont);
