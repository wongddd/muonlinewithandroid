...
__forceinline void  SendRequestServerHWID()
{
...
__forceinline void  SendRequestServerUpdate(int version)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xF4);
	BYTE byData = (BYTE)0x08;
	spe.AddData(&byData, 1, FALSE);
	WORD Index = (WORD)(version);
	spe.AddData(&Index, 2, FALSE);
	spe.Send();
}

__forceinline void  SendRequestServerAddress(int p_Index)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xF4);
	...
	spe.AddData(&Index, 2, FALSE);
	spe.Send();g_pChatListBox->AddText("", GlobalText[470], SEASON3B::TYPE_SYSTEM_MESSAGE);
	g_pChatListBox->AddText("", GlobalText[471], SEASON3B::TYPE_SYSTEM_MESSAGE);
}

__forceinline void SendRequestServerOff(BYTE State)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x20);
	spe << (BYTE)State;
...
inline void SendCheckSum(DWORD dwCheckSum)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x03);
	spe.AddNullData(1);
...
inline void SendHackingChecked(BYTE byType, BYTE byParam)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xF1);
...
inline void SendRequestLogIn(const char* p_lpszID, const char* p_lpszPassword)
{
	...
	for (int i = 0; i < SIZE_PROTOCOLSERIAL; i++)
		spe << Serial[i];
	spe.Send(TRUE);g_pChatListBox->AddText("", GlobalText[472], SEASON3B::TYPE_SYSTEM_MESSAGE);
...
__forceinline void  SendRequestCharactersList(BYTE byLanguage)
{
...
__forceinline void SendRequestCreateCharacter(char* p_ID, BYTE p_Class, BYTE p_Skin)
{
...
__forceinline void SendRequestDeleteCharacter(char* p_ID, char* p_Resident)
{
...
__forceinline void SendRequestJoinMapServer(char* p_ID)
{
...
__forceinline void SendRequestFinishLoading()
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xF3);
	spe << (BYTE)0x12;
...
__forceinline void SendPosition(int p_x, int p_y)
{
	CStreamPacketEngine spe;
...
inline void SendRequestAction(BYTE p_Action, BYTE p_Angle)\
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x18);
...
inline void SendRequestAttack(int p_Key, BYTE p_Dir)
{
	if (!FindText2(Hero->ID, "webzen"))
	{
		CStreamPacketEngine spe;
...
inline void SendRequestMagic(int p_Type, int p_Key)\
{
	if (!FindText2(Hero->ID, "webzen"))
	{
		if (gSkillManager.IsMultiTargetSkill(p_Type) == true || (abs((int)(GetTickCount() - g_dwLatestMagicTick)) > 300))
		{
...
inline void SendRequestCancelMagic(int p_Type, int p_Key)\
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x1B);
	WORD Type = (WORD)p_Type;
...
#ifndef ENABLE_EDIT
inline void SendRequestMagicAttack(int p_Type, BYTE p_x, BYTE p_y, BYTE p_Serial, int p_Count, int* p_Key, WORD p_SkillSerial)\
{
	if (!FindText2(Hero->ID, "webzen"))
	{
		CStreamPacketEngine spe;
		...
		{
			spe << (BYTE)(p_Key[i] >> 8) << (BYTE)(p_Key[i] & 0xff);
...
#ifndef ENABLE_EDIT
inline void SendRequestMagicContinue(int p_Type, BYTE p_x, BYTE p_y, BYTE p_Angle, BYTE p_Dest, BYTE p_Tpos, WORD p_TKey, BYTE* p_SkillSerial)
{
	if (!FindText2(Hero->ID, "webzen"))
	{
		CurrentSkill = p_Type;
...
inline void SendRequestMagicTeleport(bool* p_pbResult, int p_Type, int p_x, int p_y)
{
...
inline void SendRequestMagicTeleportB(bool* p_pbResult, int p_Index, int p_x, int p_y)
{
	if (Teleport)
	{
		Teleport = false;
...
inline void SendRequestTalk(WORD p_Key)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x30);
...
inline void SendRequestSell(int p_Index)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x33);
	spe << (BYTE)(p_Index);
...
__forceinline void SendRequestBuy(int Index)
{
	if (BuyCost != 0)
		return;
	CStreamPacketEngine spe;
...
__forceinline void SendRequestBuyInventory(int Index)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x35);
	spe << (BYTE)Index;
...
#ifdef CONSOLE_DEBUG
	g_ConsoleDebug->Write(MCD_SEND, "0x35 [SendRequestBuyInventory(%d)]", Index);
#endif // CONSOLE_DEBUG
}

__forceinline void SendRequestUnBreak(int p_Index, int p_AddGold)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x27);
...
__forceinline void  SendRequestRepair(int p_Index, int p_AddGold)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x34);
	spe << (BYTE)(p_Index) << (BYTE)(p_AddGold);
...
inline void SendRequestEventChip(BYTE p_Type, BYTE p_Index)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x95);
...
inline void SendRequestMutoNumber()
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x96);
	spe.Send(FALSE);
...
inline void SendRequestEventChipExit()
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x97);
	spe.Send(FALSE);
...
inline void SendRequestLenaExchange(BYTE p_byType)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x98);
	spe << (BYTE)(p_byType);
...
inline void SendRequestScratchSerial(void* p_strSerial1, void* p_strSerial2, void* p_strSerial3)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x9D);
...
inline void SendRequestServerImmigration(void* p_ResidentNumber)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x99);
...
inline void SendRequestQuestHistory()
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xA0);
	spe.Send(TRUE);
...
inline void SendRequestQuestState(BYTE p_Index, BYTE p_State)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xA2);
...
inline void SendRequestAttribute(BYTE p_Att)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x9B);
	spe << (BYTE)(p_Att);
...
// CG [0xA4][0x00]
//----------------------------------------------------------------------------
inline void SendRequestQuestMonKillInfo(BYTE byQuestIndex)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xA4);
	spe << (BYTE)0x00;
...
inline void SendQuestSelection(DWORD dwQuestIndex, BYTE byResult)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xF6);
	spe << (BYTE)0x0A;
...
inline void SendQuestSelAnswer(DWORD dwQuestIndex, BYTE bySelAnswer)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xF6);
	spe << (BYTE)0x0B;
...
inline void SendRequestQuestComplete(DWORD dwQuestIndex)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xF6);
	spe << (BYTE)0x0D;
...
inline void SendSatisfyQuestRequestFromClient(DWORD dwQuestIndex)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xF6);
	spe << (BYTE)0x10;
...
inline void SendRequestProgressQuestList()
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xF6);
	spe << (BYTE)0x1A;
...
inline void SendRequestProgressQuestRequestReward(DWORD dwQuestIndex)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xF6);
	spe << (BYTE)0x1B;
...
inline void SendRequestQuestGiveUp(DWORD dwQuestIndex)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xF6);
	spe << (BYTE)0x0F;
...
inline void SendRequestQuestByEtcEPList()
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xF6);
	spe << (BYTE)0x21;
...
inline void SendRequestGensJoining(BYTE byInfluence)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xF8);
	spe << (BYTE)0x01;
...
inline void SendRequestGensSecession()
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xF8);
	spe << (BYTE)0x03;
...
inline void SendRequestGensReward(BYTE byInfluence)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xF8);
	spe << (BYTE)0x09;
...
inline void SendRequestGensInfo_Open()
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xF8);
	spe << (BYTE)0x0B;
...
// (0xF6 0x0A)
inline void SendRequestQuestByNPCEPList()
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xF6);
	spe << (BYTE)0x30;
...
__forceinline bool SendRequestEquipmentItem(BYTE iSrcType, BYTE iSrcIndex, ITEM* pItem, BYTE iDstType, BYTE iDstIndex)
{
	BYTE splitType;
	BYTE spareBits;
	BYTE socketBits[MAX_SOCKETS];
	...
	if (!EquipmentItem && pItem)
	{
		...
		spe << (BYTE)iSrcType;
		spe << (BYTE)iSrcIndex;
		spe << (BYTE)(pItem->Type & 0xff);
		spe << (BYTE)(pItem->Level & 0xff);
		spe << (BYTE)(pItem->Durability);
		spe << (BYTE)(pItem->Option1);
		spe << (BYTE)(pItem->ExtOption);
		spe << (BYTE)splitType;
		spe << (BYTE)spareBits;
		spe << (BYTE)socketBits[0];
		spe << (BYTE)socketBits[1];
		spe << (BYTE)socketBits[2];
		spe << (BYTE)socketBits[3];
		spe << (BYTE)socketBits[4];
		spe << (BYTE)iDstType;
		spe << (BYTE)iDstIndex;
...
inline void SendRequestUse(int p_Index, int p_Target)
{
	if (IsCanUseItem())
	{
		if (EnableUse <= 0)
		{
			...
			CStreamPacketEngine spe;
			spe.Init(0xC1, 0x26);
			spe << (BYTE)((p_Index) + MAX_EQUIPMENT) << (BYTE)(p_Target);
			spe << (BYTE)g_byItemUseType;
			spe.Send(TRUE);ITEM* pItem = g_pMyInventory->FindItem(p_Index);
			if (pItem)
			{
...
#endif // ENABLE_EDIT
inline void SendRequestGetItem(int p_Key)
{
	if (SendGetItem == -1)
	{
		SendGetItem = p_Key;
...
inline void SendRequestDropItem(BYTE p_InventoryIndex, BYTE p_x, BYTE p_y)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x23);
...
inline void SendRequestPostChat(int p_Index)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x25);
	spe << (BYTE)p_Index;
...
inline void SendRequestAddPoint(BYTE p_Type)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xF3);
...
inline void SendRequestTrade(WORD p_Key)
{
...
inline void SendRequestTradeAnswer(BYTE p_Result)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x37);
	spe << (BYTE)(p_Result);
...
inline void SendRequestTradeGold(DWORD p_Gold, DWORD coin1, DWORD coin2, DWORD coin3)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x3A);
	spe.AddNullData(1);
...
inline void SendRequestTradeResult(BYTE p_Result)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x3C);
	spe << (BYTE)(p_Result);
...
inline void SendRequestTradeExit()
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x3D);
	spe.Send(TRUE);
...
inline void SendPing(int p_Key)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x71);
	spe.AddNullData(1);
...
inline void SendStoragePassword(BYTE p_byType, WORD p_wPassword, void* p_ResidentNumber)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x83);
...
void inline SendRequestParty(WORD p_Key)
{
	if (EnableMainRender)
	{
		PartyKey = p_Key;
...
inline void SendRequestPartyAnswer(BYTE p_Result)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x41);
...
inline void SendRequestPartyList()
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x42);
	spe.Send();
...
inline void SendRequestPartyLeave(BYTE p_Index)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x43);
	spe << (BYTE)(p_Index);
...
inline void SendRequestGuildMaster(BYTE p_Value)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x54);
	spe << (BYTE)(p_Value);
...
inline void SendRequestCreateGuild(BYTE GuildType, void* pGuildName, void* pGuildMark)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x55);
	spe << (BYTE)(GuildType);
...
inline void SendRequestEditGuildType(BYTE GuildType)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xE2);
	spe << (BYTE)(GuildType);
...
inline void SendRequestGuildRelationShip(BYTE RelationType, BYTE RequestType, BYTE TargetUserIndexH, BYTE TargetUserIndexL)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xE5);
...
inline void SendRequestGuildRelationShipResult(BYTE Type, BYTE RequestType, BYTE Result, BYTE TargetUserIndexH, BYTE TargetUserIndexL)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xE6);
	spe << (BYTE)(Type);
...
inline void SendRequestBanUnionGuild(void* GuildName)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xEB);
	spe << (BYTE)0x01;
...
inline void SendRequestUnionList()
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xE9);
	spe.Send();
...
inline void SendRequestEditGuildMark(void* p_Name, void* p_Mark)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x55); \
...
inline void SendRequestGuild(WORD p_Key)
{
	if (EnableMainRender)
	{
...
inline void SendRequestMix(WORD p_Type, WORD p_SubType)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x86);
	spe.AddNullData(1);
...
inline void SendRequestGemMix(BYTE iType, BYTE iLevel)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xBC);
	spe << (BYTE)0x00;
...
inline void SendRequestGemUnMix(BYTE iType, BYTE iLevel, BYTE iPos)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xBC);
	spe << (BYTE)0x01;
...
inline void SendRequestEventZoneOpenTime(BYTE byZoneType, BYTE iItemLevel)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x91);
...
void inline SendRequestDuelStart(WORD index, char* name)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xAA);
	spe << (BYTE)0x01;
...
inline void SendRequestDuelEnd()
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xAA);
	spe << (BYTE)0x03;
...
inline void SendRequestSetSalePrice(BYTE offset, INT money, int JewelBless, int JewelSoul, int JewelChaos, BYTE ShopType)
{
	CStreamPacketEngine spe;
	spe.Init( 0xC1, 0x3F);
	spe << ( BYTE)0x01;
	spe << ( BYTE)offset;
	spe << ( INT)money;
	spe << (BYTE)((DWORD)JewelBless & 0xff);
	spe << (BYTE)((DWORD)JewelBless >> 8);
	spe << (BYTE)((DWORD)JewelSoul & 0xff);
	spe << (BYTE)((DWORD)JewelSoul >> 8);
	spe << (BYTE)((DWORD)JewelChaos & 0xff);
	spe << (BYTE)((DWORD)JewelChaos >> 8);
	spe << (BYTE)ShopType;
	spe.Send( TRUE);
}

inline void SendRequestCreatePersonalShop(char* name)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x3F);
	spe << (BYTE)0x02;
...
void inline SendRequestDestoryPersonalShop()
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x3F);
	spe << (BYTE)0x03;
...
void inline SendRequestOpenPersonalShop(WORD index, char* Id)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x3F);
	spe << (BYTE)0x05;
...
inline void  SendRequestPetCommand(BYTE PetType, BYTE Command, WORD Key )
{
	CStreamPacketEngine spe;
	spe.Init( 0xC1, 0xA7);
	spe << (BYTE)(PetType);
...
inline void SendRequestPetInfo(BYTE PetType, BYTE InvType, BYTE nPos )
{
	CStreamPacketEngine spe;
	spe.Init( 0xC1, 0xA9);
	spe << (BYTE)(PetType);
...
#define SendRequestIGS_SendItemGift(lGiftItemPackageSeq, lGiftItemPriceSeq, lGiftItemDisplaySeq, lSaleZone, wItemCode, iCashType, pstrReceiveUserID, pstrGiftMessage) \
{\
...
__forceinline bool SendRequestEquippingInventoryItem(BYTE iItemPos, BYTE iValue)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xBF);
	spe << (BYTE)0x20;
...
__forceinline void SendRequestStartHelper(BYTE status)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xBF);
	spe << (BYTE)0x51;
...
__forceinline void SendRequestSaveHelperSetting(BYTE* buffer, int Size)
{
	CStreamPacketEngine spe;
	spe.Init(0xC2, 0xAE);
	spe << (BYTE)0;
...
#if defined(PBG_ADD_NEWCHAR_MONK_SKILL) && defined(_DEBUG)
...
#else
__forceinline void SendRequestRageAtt(int p_Type, int p_Key)
{
	if (!FindText2(Hero->ID, "webzen"))
	{
		CStreamPacketEngine spe;
...
__forceinline void SendRequestDarkside(WORD nSkill, int p_Key)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x4B);
...
#endif // defined(PBG_ADD_NEWCHAR_MONK_SKILL) && defined(_DEBUG)

void inline SendRequestRemoveJewelOfInventory(BYTE btIndex, BYTE btType, DWORD btValue)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x4F);
	spe << (BYTE)0x08;
...
void inline SendRequestAddJewelOfInventory(BYTE btIndex, BYTE btType, DWORD btValue)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0x4F);
	spe << (BYTE)0x09;
...
__forceinline void SendRequestMaxRanking()
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xF3);
	spe << (BYTE)(0xE7);
...
__forceinline void SendRequestRankingInfo(BYTE p_Type)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xF3);
	spe << (BYTE)(0xE6);
...
__forceinline void SendRequestPurcharseMembership(DWORD p_Type)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xF3);
	spe << (BYTE)(0xF0);
...
__forceinline void CGSendOffTrade(int p_Type)
{
	CStreamPacketEngine spe;
	spe.Init(0xC1, 0xF3);
	spe << (BYTE)(0xEB);
...
__forceinline void SendRequestMixIndex(int p_Index)
{
	CStreamPacketEngine spe;
	spe.Init(0xC2, 0x4F);
	spe << (BYTE)(0x03);
...
__forceinline void SendRequestMixState(int p_Index)
{
	CStreamPacketEngine spe;
	spe.Init(0xC2, 0x4F);
	spe << (BYTE)(0x06);
