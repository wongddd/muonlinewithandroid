...
CUIWindowMgr::CUIWindowMgr()
{
	...
	SetTimer(gwinhandle->GethWnd(), CHATCONNECT_TIMER, 15 * 1000, 0);
		...
		if (pbw)
		{
			...
			if (!(iOption & UIADDWND_FORCEPOSITION))
			{
				for (m_WindowMapIter = m_WindowMap.begin(); m_WindowMapIter != m_WindowMap.end(); ++m_WindowMapIter)
				{
					...
					{
						if (iPos_x + pbw->GetWidth() + 20 <= gwinhandle->GetScreenX())
							iPos_x += 20;
						if (iPos_y + pbw->GetHeight() + 20 <= gwinhandle->GetScreenY())
							iPos_y += 20;
						if (iPos_x + pbw->GetWidth() + 20 > gwinhandle->GetScreenX() && iPos_y + pbw->GetHeight() + 20 > gwinhandle->GetScreenY())
						{
							if (iPos_y % 10 == 9)
							{
								delete pbw;
...
void CUIWindowMgr::DoAction()
{
	if (g_dwTopWindow != 0)
	{
		if (!g_pWindowMgr->GetWindow(g_dwTopWindow))
			g_dwTopWindow = 0;
	}

	if (GetFocus() == gwinhandle->GethWnd() && PressKey(VK_F5))
	{
...
void CUIWindowMgr::HideAllWindow(BOOL bHide, BOOL bMainClose)
{
	...
	if (bHide == FALSE)
	{
		...
		{
			OpenMainWnd(gwinhandle->GetScreenX() - 250, (gwinhandle->GetScreenY() - 48) - 170);
		}
...
void CUIWindowMgr::HandleMessage()
{
	...
	case UI_MESSAGE_SELECT:
		if (m_WorkMessage.m_iParam1 != 0 && GetWindow(m_WorkMessage.m_iParam1) != NULL)
		{
			...
			if ((int)(*m_WindowArrangeListIter) != m_WorkMessage.m_iParam1 || GetFocus() == gwinhandle->GethWnd())
			{
...
void CUIBaseWindow::SetTitle(const char* pszTitle)
{
	...
	if (m_pszTitle != NULL)
	{
		if (strcmp(pszTitle, m_pszTitle) == 0)
			return;
		delete[] m_pszTitle;
	}
...
BOOL CUIBaseWindow::DoMouseAction()
{
	...
	if (CheckMouseIn(m_iPos_x, m_iPos_y, m_iWidth, m_iHeight))
	{
		if (CheckOption(UIWINDOWSTYLE_TITLEBAR) && CheckMouseIn(m_iPos_x + m_iWidth - 16, m_iPos_y + 8, 9, 9))
		{
			...
			if (!MouseOnWindow && MouseLButtonPop == true)
			{
				if (CloseCheck() == TRUE)
				{
		...
		else if (CheckOption(UIWINDOWSTYLE_MINBUTTON) && CheckMouseIn(m_iPos_x + m_iWidth - (CheckOption(UIWINDOWSTYLE_MAXBUTTON) ? 38 : 27),
			m_iPos_y + 8, 9, 9))
		{
		...
		else if (CheckOption(UIWINDOWSTYLE_MOVEABLE) && CheckOption(UIWINDOWSTYLE_TITLEBAR) && CheckMouseIn(m_iPos_x, m_iPos_y, m_iWidth, 20))
		{
		...
		else if (CheckOption(UIWINDOWSTYLE_RESIZEABLE) && CheckMouseIn(m_iPos_x + m_iWidth - 10, m_iPos_y + m_iHeight - 10, 10, 10))
		{
			if (MouseLButton == true && GetState() != UISTATE_RESIZE && g_dwActiveUIID == 0)
			{
				g_dwActiveUIID = GetUIID();
	...
	if (GetState() == UISTATE_MOVE)
	{
		if (MouseLButton == true)
		{
			...
			if (m_iPos_x + MouseX - m_iMouseClickPos_x < 0)
				m_iPos_x = 0;
			else if (m_iPos_x + m_iWidth + MouseX - m_iMouseClickPos_x > gwinhandle->GetScreenX())
				m_iPos_x = gwinhandle->GetScreenX() - m_iWidth;
			else
			...
			else if (m_iPos_y + m_iHeight + MouseY - m_iMouseClickPos_y > gwinhandle->GetScreenY())
			{
				m_iPos_y = gwinhandle->GetScreenY() - m_iHeight;
			}
			else
			{
	...
	else if (GetState() == UISTATE_RESIZE)
	{
		if (MouseLButton == true)
		{
			if (m_iResizeDir == 135)
			{
				...
				if (m_iPos_x + m_iWidth + MouseX - m_iMouseClickPos_x > gwinhandle->GetScreenX())
					m_iWidth = gwinhandle->GetScreenX() - m_iPos_x;
				else
				...
				if (m_iPos_y + m_iHeight + MouseY - m_iMouseClickPos_y > gwinhandle->GetScreenY())
					m_iHeight = gwinhandle->GetScreenY() - m_iPos_y;
				else
...
void CUIBaseWindow::Maximize()
{
	if (m_bIsMaximize == FALSE)
	{
		...
		m_iPos_y = 0;
		m_iHeight = gwinhandle->GetScreenY() - 48;
		Refresh();
		m_bIsMaximize = TRUE;
	}
	else
...
void CUIChatWindow::Init(const char* pszTitle, DWORD dwParentID)
{
	...
	m_TextInputBox.Init(gwinhandle->GethWnd(), 238, 14, 50);
	...
	//	m_PalListBox.AddText("�̸�����", 1, 1);
	//	m_PalListBox.AddText("�̸�����", 1, 1);
	//	m_PalListBox.AddText("�̸�����", 1, 1);
	//	m_PalListBox.AddText("�̸�1��", 1, 1);
	//	m_PalListBox.AddText("�̸�2��", 1, 1);
	//	m_PalListBox.AddText("�̸�3��", 1, 1);
	//	m_PalListBox.AddText("�̸�4��", 1, 1);
	//	m_PalListBox.AddText("�̸�5��", 1, 1);
	//	m_PalListBox.AddText("�̸�6��", 1, 1);
	//	m_PalListBox.AddText("�̸�7��", 1, 1);
	//	m_PalListBox.AddText("�̸�8��", 1, 1);
	//	m_PalListBox.AddText("�̸�9��", 1, 1);
...
BOOL CUIChatWindow::HandleMessage()
{
	...
	case UI_MESSAGE_TEXTINPUT:
	{
		...
		g_pMultiLanguage->ConvertWideCharToStr(strText, wstrUTF16.c_str(), CHINESE_CP);
#else
		g_pMultiLanguage->ConvertWideCharToStr(strText, wstrUTF16.c_str(), CP_UTF8);
#endif
		strncpy_s(pszText, strText.c_str(), sizeof(pszText));
		//pszText[MAX_CHATROOM_TEXT_LENGTH - 1] = 0;
	...
	case UI_MESSAGE_BTNLCLICK:
	{
		...
		case 1:
			if (m_iShowType == 1)
			{
				...
				if (m_iPos_x + m_iWidth > gwinhandle->GetScreenX()) m_iPos_x = gwinhandle->GetScreenX() - m_iWidth;
				Refresh();
			}
...
void CUIChatWindow::Lock(BOOL bFlag)
{
	if (bFlag == TRUE)
	{
		...
		{
			strncpy_s(szTitle, GlobalText[995], strlen(GlobalText[995]));
		}
		strncat(szTitle, GetTitle(), (sizeof(szTitle) - strlen(szTitle)));
		SetTitle(szTitle);
	}
	else
	{
...
void CUIPhotoViewer::RenderPhotoCharacter()
{
	...
	int WorldBackup = gMapManager->currentMap;
	gMapManager->currentMap = WD_0LORENCIA;
	...
	if (CheckOption(UIPHOTOVIEWER_CANCONTROL))
	{
		...
		gluPerspective2(1.f, (float)(m_iWidth * g_fScreenRate_x) / (float)(141 * g_fScreenRate_y), 2000, 20000);
	}
	else
	{
		glViewport2(m_iPos_x * g_fScreenRate_x, m_iPos_y * g_fScreenRate_y, m_iWidth * g_fScreenRate_x, m_iHeight * g_fScreenRate_y);
		gluPerspective2(1.f, (float)(m_iWidth * g_fScreenRate_x) / (float)(m_iHeight * g_fScreenRate_y), 2000, 20000);
	}
	...
	{
		glTranslatef(-o->Position[0], -o->Position[1], -((o->Position[2] - 50.0f)) + m_fCurrentHeight);
	}
	else
	{
		glTranslatef(-o->Position[0], -o->Position[1], -(o->Position[2] + m_fCurrentHeight));
	}
	...
	Vector(0.9, 0.9, 0.9, o->Light);
	Vector(1, 1, 1, m_PhotoHelper.Light);

	c->HideShadow = true;
	c->SafeZone = (c->Wing.Type != -1 && m_iSettingAnimation > AT_HEALING1);
	...
	gMapManager->currentMap = WorldBackup;
...
{
	if (m_PhotoHelper.Live == true && (m_PhotoHelper.movementType == Movement::Fenrir || m_PhotoHelper.movementType == Movement::Horse || m_PhotoHelper.movementType == Movement::Dinorant || m_PhotoHelper.movementType == Movement::Uniria))
	{
...
void CUIPhotoViewer::CopyPlayer()
{
	...
	if (Hero->Change == FALSE)
	{
		...
		if (CompareItemEqual(&m_PhotoChar.Core, &Hero->Core) == FALSE)
			bChangeWing = TRUE;
#endif // SHUTDOWN_CORE_WING4

#ifdef SYSTEM_FLAG_NAT
		if (CompareItemEqual(&m_PhotoChar.Flag, &Hero->Flag) == FALSE)
			bChangeFlag = TRUE;
#endif // SYSTEM_FLAG_NAT
	...
	else	// ���� ����
	{
		...
		if (CompareItemEqual(&m_PhotoChar.BodyPart[BODYPART_HELM], &CharacterMachine->Equipment[EQUIPMENT_HELM], MODEL_BODY_HELM + gCharacterManager.GetSkinModelIndex(Hero->Class)) == FALSE)
			bChangeArmor = TRUE;
		else if (CompareItemEqual(&m_PhotoChar.BodyPart[BODYPART_ARMOR], &CharacterMachine->Equipment[EQUIPMENT_ARMOR], MODEL_BODY_ARMOR + gCharacterManager.GetSkinModelIndex(Hero->Class)) == FALSE)
			bChangeArmor = TRUE;
		else if (CompareItemEqual(&m_PhotoChar.BodyPart[BODYPART_PANTS], &CharacterMachine->Equipment[EQUIPMENT_PANTS], MODEL_BODY_PANTS + gCharacterManager.GetSkinModelIndex(Hero->Class)) == FALSE)
			bChangeArmor = TRUE;
		else if (CompareItemEqual(&m_PhotoChar.BodyPart[BODYPART_GLOVES], &CharacterMachine->Equipment[EQUIPMENT_GLOVES], MODEL_BODY_GLOVES + gCharacterManager.GetSkinModelIndex(Hero->Class)) == FALSE)
			bChangeArmor = TRUE;
		else if (CompareItemEqual(&m_PhotoChar.BodyPart[BODYPART_BOOTS], &CharacterMachine->Equipment[EQUIPMENT_BOOTS], MODEL_BODY_BOOTS + gCharacterManager.GetSkinModelIndex(Hero->Class)) == FALSE)
			bChangeArmor = TRUE;
...
#ifdef SHUTDOWN_CORE_WING4
		if (CompareItemEqual(&m_PhotoChar.Core, &CharacterMachine->Equipment[EQUIPMENT_WING_CORE], -1) == FALSE)
			bChangeWing = TRUE;
#endif // SHUTDOWN_CORE_WING4


#ifdef SYSTEM_FLAG_NAT
		if (CompareItemEqual(&m_PhotoChar.Flag, CharacterMachine->GetOnlyEquipment(EQUIPMENT_FLAG_NAT), -1) == FALSE)
			bChangeFlag = TRUE;
#endif // SYSTEM_FLAG_NAT
...
#ifdef SYSTEM_FLAG_NAT
	if (bChangeArmor == FALSE && bChangeWeapon == FALSE && bChangeWing == FALSE && bChangeHelper == FALSE && bChangeFlag == FALSE)
		return;
#else
	...
	if (Hero->Change == FALSE)
	{
...
#ifdef SHUTDOWN_CORE_WING4
			memcpy(&m_PhotoChar.Core, &Hero->Core, sizeof(PART_t));
#endif // SHUTDOWN_CORE_WING4
		...
		if (bChangeFlag == TRUE)
		{
			memcpy(&m_PhotoChar.Flag, &Hero->Flag, sizeof(PART_t));
		}
#endif // SYSTEM_FLAG_NAT
	...
	else
	{
		...
		{
			DeleteCloth(&m_PhotoChar, NULL, NULL);
			m_PhotoChar.BodyPart[BODYPART_HEAD].Type = MODEL_BODY_HELM + Hero->GetSkinModelIndex();
			SetItemToPhoto(&m_PhotoChar.BodyPart[BODYPART_HELM], &CharacterMachine->Equipment[EQUIPMENT_HELM], MODEL_BODY_HELM + Hero->GetSkinModelIndex());
			SetItemToPhoto(&m_PhotoChar.BodyPart[BODYPART_ARMOR], &CharacterMachine->Equipment[EQUIPMENT_ARMOR], MODEL_BODY_ARMOR + Hero->GetSkinModelIndex());
			SetItemToPhoto(&m_PhotoChar.BodyPart[BODYPART_PANTS], &CharacterMachine->Equipment[EQUIPMENT_PANTS], MODEL_BODY_PANTS + Hero->GetSkinModelIndex());
			SetItemToPhoto(&m_PhotoChar.BodyPart[BODYPART_GLOVES], &CharacterMachine->Equipment[EQUIPMENT_GLOVES], MODEL_BODY_GLOVES + Hero->GetSkinModelIndex());
			SetItemToPhoto(&m_PhotoChar.BodyPart[BODYPART_BOOTS], &CharacterMachine->Equipment[EQUIPMENT_BOOTS], MODEL_BODY_BOOTS + Hero->GetSkinModelIndex());
	...
	if (bChangeHelper == TRUE || bChangeWeapon == TRUE)
	{
		...
		if (m_PhotoChar.Helper.Type != -1)
		{
			switch (m_PhotoChar.Helper.Type - MODEL_HELPER)
			{
			case 0:
				CreateBugSub(MODEL_HELPER, m_PhotoChar.Object.Position, &m_PhotoChar.Object, &m_PhotoHelper);
				break;
			case 2:
				CreateBugSub(MODEL_UNICON, m_PhotoChar.Object.Position, &m_PhotoChar.Object, &m_PhotoHelper);
				break;
			case 3:
				CreateBugSub(MODEL_PEGASUS, m_PhotoChar.Object.Position, &m_PhotoChar.Object, &m_PhotoHelper);
				break;
			case 4:
				CreateBugSub(MODEL_DARK_HORSE, m_PhotoChar.Object.Position, &m_PhotoChar.Object, &m_PhotoHelper);
				break;
			case 37:
			...
			default:
			{
				if (isGoBoidItem(m_PhotoChar.Helper.Type - MODEL_ITEM))
				{
					CreateBugSub(m_PhotoChar.Helper.Type, m_PhotoChar.Object.Position, &m_PhotoChar.Object, &m_PhotoHelper);
				}
			}
			break;
			}
...
void CUIPhotoViewer::SetEquipmentPacket(DWORD* Equipment)
{
	if (m_bIsInitialized == FALSE)
		return;
	...
	ChangeCharacterExt(0, Equipment, &m_PhotoChar, &m_PhotoHelper);
...
void CUIPhotoViewer::SetHeight(float fHeight)
{
...
void CUILetterWriteWindow::Init(const char* pszTitle, DWORD dwParentID)
{
	...
	m_MailtoInputBox.Init(gwinhandle->GethWnd(), 238, 14, 50);
	...
	m_TitleInputBox.Init(gwinhandle->GethWnd(), 238, 14, 50);
	...
	m_TextInputBox.Init(gwinhandle->GethWnd(), 238, 135, 50);
...
BOOL CUILetterWriteWindow::HandleMessage()
{
	...
	case UI_MESSAGE_BTNLCLICK:
	{
		...
		case 1:
			if (m_bIsSend == FALSE)
			{
				wchar_t* szTitleUTF16 = new wchar_t[MAX_LETTER_TITLE_LENGTH_UTF16 + 1];
				wchar_t* szTextUTF16 = new wchar_t[MAX_LETTER_TEXT_LENGTH_UTF16 + 1];
				...
				for (k = 0; k < MAX_LETTER_TITLE_LENGTH_UTF16; k++)
				...
				// Aseg�rate de que est�n terminados en nulo
...
#ifdef CHINESE_LANGUAGE
				g_pMultiLanguage->ConvertWideCharToStr(strTitle, wstrTitle.c_str(), CHINESE_CP);
...
#endif
	pCRSocket->m_WSClient.Create(gwinhandle->GethWnd());
...
void CUITextInputWindow::Init(const char* pszTitle, DWORD dwParentID)
{
	...
	m_TextInputBox.Init(gwinhandle->GethWnd(), 80, 14, 10);
	m_TextInputBox.SetParentUIID(m_dwUIID);
	m_TextInputBox.SetFont(g_hFont);
...
void CUITextInputWindow::ReturnText()
{
	...
	g_pWindowMgr->SendUIMessageToWindow(m_dwReturnWindowUIID, UI_MESSAGE_TXTRETURN, GetUIID(), (DWORD_PTR)pszReturnText);
	g_pWindowMgr->SendUIMessage(UI_MESSAGE_CLOSE, GetUIID(), 0);
}
...
void CUIFriendMenu::Init()
{
	...
	m_iFriendMenuPos_y = RenderFrameY + 459 - 24;
	m_iFriendMenuHeight = 18;
	SetPosition(RenderFrameX + 582, m_iFriendMenuPos_y);
	SetSize(52, 0);
	m_fLineHeight = 0;
	m_WindowListSelectIter = m_WindowList.end();
...
void CUIFriendMenu::RenderFriendButton()
{
	...
	if (g_pWindowMgr->GetFriendMainWindow() != NULL)
	{
		RenderBitmap(BITMAP_INTERFACE_EX + 18, m_iPos_x, m_iFriendMenuPos_y, m_iWidth, m_iFriendMenuHeight, 0.f, 0.f, m_iWidth / 64.f, m_iFriendMenuHeight / 32.f);

		if (g_pFriendMenu->IsNewChatAlert() && bIsAlertTime)
		{
			RenderBitmap(BITMAP_INTERFACE_EX + 17, m_iPos_x + 7, m_iFriendMenuPos_y + 3, 25.f, 14.f, 0.f, 14.f / 32.f, 25.f / 32.f, 14.f / 32.f);
		}
	...
	else
	{
		if (g_pFriendMenu->IsNewChatAlert() && bIsAlertTime)
		{
			RenderBitmap(BITMAP_INTERFACE_EX + 17, m_iPos_x + 4, m_iFriendMenuPos_y + 2, 25.f, 14.f, 0.f, 0.f / 32.f, 25.f / 32.f, 14.f / 32.f);
		}
		if (g_pFriendMenu->IsNewMailAlert())
		{
			if (bIsAlertTime)
			{
				RenderBitmap(BITMAP_INTERFACE_EX + 16, m_iPos_x + 33, m_iFriendMenuPos_y + 5, 15.f, 9.f, 0.f, 0.f, 15.f / 16.f, 9.f / 16.f);
				if (m_iBlinkTemp % 24 == 11) ++m_iLetterBlink;
				if (m_iLetterBlink > 5)
				{
					m_iLetterBlink = 0;
