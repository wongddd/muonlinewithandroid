...
bool SEASON3B::CNewUITextInputMsgBox::Create(DWORD dwMsgBoxType, DWORD dwInputType, int iInputBoxWidth, int iInputBoxHeight, int iTextLimit, bool bIsPassword)
{
	...
	x = ((SCREEN_WIDTH - MSGBOX_WIDTH) / 2);
	y = 100;
	width = MSGBOX_WIDTH;
	height = MSGBOX_TOP_HEIGHT + MSGBOX_MIDDLE_HEIGHT + MSGBOX_BOTTOM_HEIGHT;
	if (CNewUIMessageBoxBase::Create(x, y, width, height) == NULL)
	{
		return false;
	}
	...
	if (m_pInputBox)
	{
		m_pInputBox->Init(gwinhandle->GethWnd(), iInputBoxWidth, iInputBoxHeight, iTextLimit, bIsPassword);
...
CALLBACK_RESULT SEASON3B::CNewUITextInputMsgBox::LButtonUp(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	if (pMsgBox)
	{
		...
		{
		case MSGBOX_COMMON_TYPE_OK:
			if (pMsgBox->m_BtnOk.IsMouseIn() != NULL)
			{
		...
		case MSGBOX_COMMON_TYPE_OKCANCEL:
			...
			if (pMsgBox->m_BtnCancel.IsMouseIn() != NULL)
			{
...
bool SEASON3B::CNewUITextInputMsgBox::Update()
{
	...
	if (m_pInputBox)
	{
		...
		if (false == m_pInputBox->HaveFocus() && g_MessageBox->GetRelatedWnd() != gwinhandle->GethWnd())
		{
			g_MessageBox->SetRelatedWnd(gwinhandle->GethWnd());
		}
	}
...
bool SEASON3B::CNewUITextInputMsgBox::Render()
{
	...
	SEASON3B::window_emergent(this, m_MsgTextList.size());
...
CALLBACK_RESULT SEASON3B::CNewUIKeyPadMsgBox::LButtonUp(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	if (pMsgBox)
	{
		for (int i = 0; i < MAX_KEYPADINPUT; ++i)
		{
			if (pMsgBox->m_BtnKeyPad[i].IsMouseIn() != NULL)
			{
				leaf::xstreambuf buf;
		...
		if (pMsgBox->m_BtnDeleteKeyPad.IsMouseIn() != NULL)
		{
...
{
	SEASON3B::window_emergent(this, MSGBOX_MIDDLE_FRAME_NUM);
}
...
CALLBACK_RESULT SEASON3B::CUseFruitCheckMsgBox::LButtonUp(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	if (pMsgBox)
	{
		if (pMsgBox->m_BtnAdd.IsMouseIn() != NULL)
		{
			g_MessageBox->SendEvent(pOwner, MSGBOX_EVENT_USER_CUSTOM_USE_FRUIT_ADD);
			return CALLBACK_BREAK;
		}
		if (pMsgBox->m_BtnMinus.IsMouseIn() != NULL)
		{
...
{
	SEASON3B::window_emergent(this, 1);
}
...
CALLBACK_RESULT SEASON3B::CGemIntegrationMsgBox::LButtonUp(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	if (pMsgBox)
	{
		if (pMsgBox->m_BtnUnity.IsMouseIn() != NULL)
		{
			g_MessageBox->SendEvent(pOwner, MSGBOX_EVENT_USER_CUSTOM_GEM_UNITY);
			return CALLBACK_BREAK;
		}
		if (pMsgBox->m_BtnDisjoint.IsMouseIn() != NULL)
		{
...
{
	SEASON3B::window_emergent(this, m_nMiddleCount);
}
...
CALLBACK_RESULT SEASON3B::CGemIntegrationDisjointMsgBox::BlessingBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	if (COMGEM::m_UnmixTarList.IsEmpty() != NULL)
	{
		g_pChatListBox->AddText("", GlobalText[1818], SEASON3B::TYPE_ERROR_MESSAGE);
		COMGEM::GetBack();
...
CALLBACK_RESULT SEASON3B::CGemIntegrationDisjointMsgBox::DisjointBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	if (pUT)
	{
		...
		if (pMsgBox)
		{
			...
			ITEM* pItem = g_pMyInventory->FindItem(pUT->m_iInvenIdx, 0);
...
{
	SEASON3B::window_emergent(this, m_iMiddleFrameCount);
}
...
{
#if MAIN_UPDATE > 603
	SEASON3B::window_emergent(this, 5, 0);
...
CALLBACK_RESULT SEASON3B::CSystemMenuMsgBox::LButtonUp(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	if (pMsgBox)
	{
		if (pMsgBox->m_BtnGameOver.IsMouseIn() != NULL)
		{
		...
		if (pMsgBox->m_BtnChooseServer.IsMouseIn() != NULL)
		{
		...
		if (pMsgBox->m_BtnChooseCharacter.IsMouseIn() != NULL)
		{
		...
		if (pMsgBox->m_BtnOption.IsMouseIn() != NULL)
		{
...
bool SEASON3B::CChaosMixMenuMsgBox::Create(float fPriority)
{
	...
	//x = (SCREEN_WIDTH / 2) - (MSGBOX_WIDTH / 2);
	x = to_pos_right(640-(200*2));
	y = 60;
	width = MSGBOX_WIDTH;
	height = MSGBOX_TOP_HEIGHT + (MIDDLE_COUNT * MSGBOX_MIDDLE_HEIGHT) + MSGBOX_BOTTOM_HEIGHT;

	CNewUIMessageBoxBase::Create(x, y, width, height, fPriority,false);
...
CALLBACK_RESULT SEASON3B::CDialogMsgBox::LButtonUp(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	if (pMsgBox)
	{
		if (pMsgBox->m_BtnEnd.IsMouseIn() != NULL)
		{
...
void SEASON3B::CDialogMsgBox::RenderFrame()
{
	...
	SEASON3B::window_emergent(this, ((iMiddleFrameCount > 0) ? iMiddleFrameCount : 0));
}
...
bool SEASON3B::CCursedTempleProgressMsgBox::Update()
{
	...
	if (CheckHeroAction() == NULL || g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_CURSEDTEMPLE_GAMESYSTEM) == NULL)
	{
		g_MessageBox->SendEvent(this, MSGBOX_EVENT_USER_CUSTOM_PROGRESS_CLOSINGPROCESS);
	}
...
bool SEASON3B::CCursedTempleProgressMsgBox::CheckHeroAction()
{
	...
	if (!(action >= PLAYER_SET && action <= PLAYER_STOP_RIDE_WEAPON)
		...
		&& !(action == PLAYER_FENRIR_STAND_WEAPON_TWO)
		&& !(action == PLAYER_FENRIR_STAND_WEAPON_RIGHT)
		&& !(action == PLAYER_FENRIR_STAND_WEAPON_LEFT)
		&& !(action == PLAYER_DARKLORD_STAND)
		&& !(action == PLAYER_STOP_RIDE_HORSE)
		&& !(action == PLAYER_HIT_STRIKE)
...
void SEASON3B::CDuelMsgBox::RenderFrame()
{
	...
	SEASON3B::window_emergent(this, 7);
...
CALLBACK_RESULT SEASON3B::CDuelMsgBox::LButtonUp(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	if (pMsgBox)
	{
		...
		else if (pMsgBox->m_BtnCancel.IsMouseIn() != NULL)
		{
...
{
	SEASON3B::window_emergent(this, m_iMiddleCount);
}
...
CALLBACK_RESULT SEASON3B::CPersonalShopItemValueMsgBoxLayout::ProcessOk(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	{
		if (g_pMyShopInventory->IsEnablePersonalShop() != NULL)
		{
		...
		if (pPickedItem)
		{
			...
			if (InvenIndex != -1)
			{
				...
				SendRequestSetSalePrice(byIndex, iInputZen, 0, 0, 0, 0);
			...
			AddPersonalItemPrice(MAX_MY_INVENTORY_INDEX + iTargetIndex, 0, iInputZen, 0, 0, 0, g_IsPurchaseShop);
		}
		else
		{
			...
			BYTE byIndex = iSourceIndex;
			SendRequestSetSalePrice(byIndex, iInputZen, 0, 0, 0, 0);
			AddPersonalItemPrice(iSourceIndex, 0, iInputZen, 0, 0, 0, g_IsPurchaseShop);
		}
	}
...
CALLBACK_RESULT SEASON3B::CStorageLockKeyPadMsgBoxLayout::OkBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	if (pMsgBox->GetInputSize() == pMsgBox->GetInputLimit())
	{
		if (pMsgBox->IsAllSameNumber() != NULL)
		{
			pMsgBox->ClearInput();
			g_MessageBox->SendEvent(pOwner, MSGBOX_EVENT_DESTROY);
...
CALLBACK_RESULT SEASON3B::CLuckyTradeMenuMsgBox::LButtonUp(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	if (pMsgBox)
	{
		if (pMsgBox->m_BtnTrade.IsMouseIn() != NULL)
		{
			g_MessageBox->SendEvent(pOwner, MSGBOX_EVENT_USER_CUSTOM_LUCKYITEM_TRADE);
			return CALLBACK_BREAK;
		}
		if (pMsgBox->m_BtnRefinery.IsMouseIn() != NULL)
		{
...
CALLBACK_RESULT SEASON3B::CTrainerMenuMsgBox::LButtonUp(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	if (pMsgBox)
	{
		if (pMsgBox->m_BtnRecover.IsMouseIn() != NULL)
		{
			g_MessageBox->SendEvent(pOwner, MSGBOX_EVENT_USER_CUSTOM_TRAINER_MENU_RECOVER);
			return CALLBACK_BREAK;
		}
		if (pMsgBox->m_BtnRevive.IsMouseIn() != NULL)
		{
...
CALLBACK_RESULT SEASON3B::CTrainerRecoverMsgBox::LButtonUp(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	if (pMsgBox)
	{
		if (pMsgBox->m_BtnRecoverDarkSpirit.IsMouseIn() != NULL)
		{
			g_MessageBox->SendEvent(pOwner, MSGBOX_EVENT_USER_CUSTOM_TRAINER_MENU_RECOVER_DARKSPRIT);
			return CALLBACK_BREAK;
		}
		if (pMsgBox->m_BtnRecoverDarkHorse.IsMouseIn() != NULL)
		{
...
CALLBACK_RESULT SEASON3B::CResetCharacterPointMsgBox::LButtonUp(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	if (pMsgBox)
	{
		if (pMsgBox->m_ResetCharacterPointBtn.IsMouseIn() != NULL)
		{
...
{
	SEASON3B::window_emergent(this, 5);
}
...
bool SEASON3B::CAddPointsReceiptMsgBoxLayout::SetLayout()
{
	CNewUITextInputMsgBox* pMsgBox = GetMsgBox();
	if (0 == pMsgBox)
		return false;
	...
	int InputIndex = g_pCharacterInfoWindow->GetInputIndex();
	...
	{
	case 0:
		sprintf_s(strText, "%s %s %s", GlobalText[3505], GlobalText[682], GlobalText[166]); // Thêm /điểm /Sức mạnh
		break;
	case 1:
		sprintf_s(strText, "%s %s %s", GlobalText[3505], GlobalText[682], GlobalText[167]); // Thêm /điểm /Nhanh nhẹn
		break;
	case 2:
		sprintf_s(strText, "%s %s %s", GlobalText[3505], GlobalText[682], GlobalText[169]); // Thêm /điểm /Thể lực
		break;
	case 3:
		sprintf_s(strText, "%s %s %s", GlobalText[3505], GlobalText[682], GlobalText[168]); // Thêm /điểm /Năng lượng
		break;
	case 4:
		sprintf_s(strText, "%s %s %s", GlobalText[3505],GlobalText[682],GlobalText[1900]); // Thêm /điểm /Mệnh Lệnh
		break;
	}

	pMsgBox->SetInputBoxOption(UIOPTION_NUMBERONLY | UIOPTION_PAINTBACK);
	pMsgBox->AddMsg(strText, ARGB(255, 33, 194, 28), 1);
	pMsgBox->AddCallbackFunc(CAddPointsReceiptMsgBoxLayout::ReturnDown, MSGBOX_EVENT_PRESSKEY_RETURN);
	pMsgBox->AddCallbackFunc(CAddPointsReceiptMsgBoxLayout::OkBtnDown, MSGBOX_EVENT_USER_COMMON_OK);
	pMsgBox->AddCallbackFunc(CAddPointsReceiptMsgBoxLayout::CancelBtnDown, MSGBOX_EVENT_USER_COMMON_CANCEL);

	return true;
}

CALLBACK_RESULT SEASON3B::CAddPointsReceiptMsgBoxLayout::ReturnDown(CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	return ProcessOk(pOwner, xParam);
}

CALLBACK_RESULT SEASON3B::CAddPointsReceiptMsgBoxLayout::OkBtnDown(CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	return ProcessOk(pOwner, xParam);
}

CALLBACK_RESULT SEASON3B::CAddPointsReceiptMsgBoxLayout::CancelBtnDown(CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	PlayBuffer(SOUND_CLICK01);
	g_MessageBox->SendEvent(pOwner, MSGBOX_EVENT_DESTROY);
...
CALLBACK_RESULT SEASON3B::CAddPointsReceiptMsgBoxLayout::ProcessOk(CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	CNewUITextInputMsgBox* pMsgBox = dynamic_cast<CNewUITextInputMsgBox*>(pOwner);
	...
	if (InputIndex >= 0 && InputIndex < 5)
	{
		...
		g_pNewUICommandServer->SendCommand(2 + InputIndex, "%d", InputStat);
	}
	PlayBuffer(SOUND_CLICK01);
	g_MessageBox->SendEvent(pOwner, MSGBOX_EVENT_DESTROY);
...
SEASON3B::CNewUIItem3DInputMsgBox::CNewUIItem3DInputMsgBox()
{
	m_dwPersonalType = 0;
	for (int l = 0; l < 4; l++)
	{
...
bool SEASON3B::CNewUIItem3DInputMsgBox::Create(DWORD dwMsgBoxType, DWORD dwInputType, int iInputBoxWidth, int iInputBoxHeight, int iLimitText, bool bIsPassword)
{
	int width = 190;
	int height = 230;

	int x = ((GetWindowsX - width) / 2);
	int y = 100;
	...
	for (int l = 0; l < 4; l++)
	{
		SAFE_DELETE(m_pInputBox[l]);
		m_pInputBox[l] = new CUITextInputBox;

		if (m_pInputBox[l])
		{
			m_pInputBox[l]->Init(gwinhandle->GethWnd(), iInputBoxWidth, iInputBoxHeight, iLimitText, bIsPassword);
			x = GetPos().x + ((GetSize().cx - iInputBoxWidth) * 0.5);
			y = GetPos().y + 95.f + MSGBOX_MIDDLE_HEIGHT;

			m_pInputBox[l]->SetPosition(x, y + l * MSGBOX_MIDDLE_HEIGHT);
			m_pInputBox[l]->SetTextColor(255, 255, 230, 210);
			m_pInputBox[l]->SetBackColor(255, 0, 0, 0);
			m_pInputBox[l]->SetFont(g_hFont);
			m_pInputBox[l]->SetState(UISTATE_NORMAL);
			//m_pInputBox[l]->GiveFocus();
		}
	}
	...
	AddCallbackFunc(SEASON3B::CNewUIItem3DInputMsgBox::LButtonUp, MSGBOX_EVENT_MOUSE_LBUTTON_UP);
	AddCallbackFunc(SEASON3B::CNewUIItem3DInputMsgBox::Close, MSGBOX_EVENT_USER_COMMON_OK);
	AddCallbackFunc(SEASON3B::CNewUIItem3DInputMsgBox::Close, MSGBOX_EVENT_PRESSKEY_ESC);

	return true;
}

void SEASON3B::CNewUIItem3DInputMsgBox::Release()
{
	CNewUIMessageBoxBase::Release();
...
CALLBACK_RESULT SEASON3B::CNewUIItem3DInputMsgBox::Close(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
...
CALLBACK_RESULT SEASON3B::CNewUIItem3DInputMsgBox::LButtonUp(CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	CNewUIItem3DInputMsgBox* pMsgBox = dynamic_cast<CNewUIItem3DInputMsgBox*>(pOwner);
...
void SEASON3B::CNewUIItem3DInputMsgBox::AddMsg(const type_string& strMsg, DWORD dwColor, BYTE byFontType)
{
	int iOrigSize = m_MsgTextList.size();
	int iLine = SeparateText(strMsg, dwColor, byFontType);
	int iSize = m_MsgTextList.size();
	...
	if (iSize > 1)
	{
		...
		for (int l = 0; l < 4; l++)
		{
			if (m_pInputBox[l])
			{
				m_pInputBox[l]->SetPosition(m_pInputBox[l]->GetPosition_x(), m_pInputBox[l]->GetPosition_y() + (iLine * MSGBOX_MIDDLE_HEIGHT));
			}
		}
	}
}

void SEASON3B::CNewUIItem3DInputMsgBox::Set3DItem(ITEM* pItem)
{
	if (pItem)
	{
		memcpy(&m_Item, pItem, sizeof(ITEM));
...
bool SEASON3B::CNewUIItem3DInputMsgBox::Update()
{
	switch (m_dwMsgBoxType)
	{
	case MSGBOX_COMMON_TYPE_OK:
	...
	for (int l = 0; l < 4; l++)
	{
		if (m_pInputBox[l])
		{
			m_pInputBox[l]->DoAction();

			if (m_pInputBox[l]->HaveFocus() && g_MessageBox->GetRelatedWnd() != m_pInputBox[l]->GetHandle())
			{
				g_MessageBox->SetRelatedWnd(m_pInputBox[l]->GetHandle());
			}

			if (false == m_pInputBox[l]->HaveFocus() && g_MessageBox->GetRelatedWnd() != gwinhandle->GethWnd())
			{
				count_of_have++;
			}
		}
...
bool SEASON3B::CNewUIItem3DInputMsgBox::Render()
{
	EnableAlphaTest();
	glColor4f(1.0f, 1.0f, 1.0f, 1.0f);

	float RenderFrameX = GetPos().x;
	float RenderFrameY = GetPos().y;
	float RenderSizeX = GetSize().cx;
	float height = GetSize().cy - MSGBOX_BACK_BLANK_HEIGHT;

#if MAIN_UPDATE > 603
	SEASON3B::window_emergent(this, 9);
#else
	SEASON3B::RenderImageF(IMAGE_BG_SERVER_INTERFACE_250, RenderFrameX, RenderFrameY, RenderSizeX, 230.0, 0.0, 0.0, 250.0, 250.0);
#endif

	SEASON3B::RenderImageF(IMAGE_PURCHARSE_CAPTION_1, RenderFrameX + ((RenderSizeX - MSGBOX_3DITEM_WIDTH) / 2.f), RenderFrameY + 20.f, MSGBOX_3DITEM_WIDTH, MSGBOX_3DITEM_HEIGHT, 529, 231, 53.0, 53.0);

	SEASON3B::RenderImageF(IMAGE_PURCHARSE_CAPTION_1, m_pInputBox[0]->GetPosition_x() - 20.f, m_pInputBox[0]->GetPosition_y(), 10.f, 54.f, 20.0, 0.0, 18.0, 96.0);

	int render_row = m_pInputBox[0]->GetPosition_x() + m_pInputBox[0]->GetWidth();

	SEASON3B::RenderImageF(CNewUIQuestProgress::IMAGE_QP_BTN_R, render_row + 30.f, m_pInputBox[0]->GetPosition_y() - 3.f, 13.f, 14.f, 0.0, 0.0, 17, 18.f);

	RenderTexts();

	const char* _text[] = { GlobalText[100], TextGlobal[58], TextGlobal[59], TextGlobal[60], GlobalText[101], };

	g_pRenderText->RenderText(render_row + 4.f, m_pInputBox[0]->GetPosition_y(), _text[m_dwPersonalType]);

	for (int l = 0; l < 4; l++)
	{
		if (m_pInputBox[l])
			m_pInputBox[l]->Render();
		SEASON3B::RenderImageF(IMAGE_PURCHARSE_CAPTION_1, RenderFrameX + ((RenderSizeX - MSGBOX_3DITEM_WIDTH) / 2.f), RenderFrameY + 20.f, MSGBOX_3DITEM_WIDTH, MSGBOX_3DITEM_HEIGHT, 527, 229, 55.0, 55.0);
	}
...
void SEASON3B::CNewUIItem3DInputMsgBox::Render3D()
{
	Script_Item* item_info = GMItemMng->find(m_Item.Type);

	float width = MSGBOX_3DITEM_WIDTH;
	float hight = MSGBOX_3DITEM_HEIGHT;
	float x = GetPos().x + ((GetSize().cx - width) * 0.5);
	float y = GetPos().y + 20;

	if (item_info && item_info->SubIndex != -1)
	{
		width = item_info->Width * 20;
		hight = item_info->Height * 20;
		x = GetPos().x + ((190.0 - width) * 0.5);
		y += (MSGBOX_3DITEM_HEIGHT - hight) / 2.f;
	}

	RenderItem3D(x, y, width, hight, m_Item.Type, m_Item.Level, m_Item.Option1, m_Item.ExtOption, false);
}

bool SEASON3B::CNewUIItem3DInputMsgBox::IsVisible() const
{
	return true;
}

bool SEASON3B::CNewUIItem3DInputMsgBox::UpdateMouse()
{
	int render_row = m_pInputBox[0]->GetPosition_x() + m_pInputBox[0]->GetWidth();

	if (SEASON3B::CheckMouseIn(render_row + 30.f, m_pInputBox[0]->GetPosition_y() - 3.f, 13.f, 14.f) && SEASON3B::IsRelease(VK_LBUTTON))
	{
#if MAIN_UPDATE > 303
		if (m_dwPersonalType < 3)
			m_dwPersonalType++;
		else
			m_dwPersonalType = 0;
#else
		if (m_dwPersonalType < 1)
			m_dwPersonalType++;
		else
			m_dwPersonalType = 0;
#endif
...
DWORD SEASON3B::CNewUIItem3DInputMsgBox::GetMsgBoxType()
{
	return m_dwMsgBoxType;
}

DWORD SEASON3B::CNewUIItem3DInputMsgBox::GetPersonalType()
{
	return m_dwPersonalType;
}

void SEASON3B::CNewUIItem3DInputMsgBox::GetInputBoxText(unicode::t_char* strText, int index)
{
	if (m_pInputBox[index])
	{
		m_pInputBox[index]->GetText(strText);
	}
}

void SEASON3B::CNewUIItem3DInputMsgBox::SetInputBoxOption(int iOption, int index)
{
	if (m_pInputBox[index])
	{
		m_pInputBox[index]->SetOption(iOption);
	}
}

void SEASON3B::CNewUIItem3DInputMsgBox::SetInputBoxPosition(int x, int y, int index)
{
	if (m_pInputBox[index])
	{
		m_pInputBox[index]->SetPosition(x, y);
	}
}

void SEASON3B::CNewUIItem3DInputMsgBox::SetInputBoxSize(int width, int height, int index)
{
	if (m_pInputBox[index])
	{
		m_pInputBox[index]->SetSize(width, height);
	}
}

int SEASON3B::CNewUIItem3DInputMsgBox::SeparateText(const type_string& strMsg, DWORD dwColor, BYTE byFontType)
{
	SIZE TextSize;
	size_t TextExtentWidth;
	int iLine = 0;
...
void SEASON3B::CNewUIItem3DInputMsgBox::SetButtonInfo()
{
	float x, y, width, height;
	...
	case MSGBOX_COMMON_TYPE_OK:
		x = GetPos().x + (GetSize().cx / 2) - (MSGBOX_BTN_WIDTH / 2);
		y = GetPos().y + GetSize().cy - (23 + MSGBOX_BTN_BOTTOM_BLANK);
		width = MSGBOX_BTN_WIDTH;
		height = 23;
		m_BtnOk.SetInfo(BITMAP_btn_empty_very_small, x, y, width, height);
		m_BtnOk.SetText(GlobalText[228]);
		break;
	case MSGBOX_COMMON_TYPE_OKCANCEL:
		x = GetPos().x + (((GetSize().cx / 2) - MSGBOX_BTN_WIDTH) / 2);
		y = GetPos().y + GetSize().cy - (MSGBOX_BTN_EMPTY_SMALL_HEIGHT + MSGBOX_BTN_BOTTOM_BLANK);
		width = MSGBOX_BTN_WIDTH;
		height = 23;
		m_BtnOk.SetInfo(BITMAP_btn_empty_very_small, x, y, width, height);
		m_BtnOk.SetText(GlobalText[228]);
		...
		width = MSGBOX_BTN_WIDTH;
		height = 23;
		m_BtnCancel.SetInfo(BITMAP_btn_empty_very_small, x, y, width, height);
		m_BtnCancel.SetText(GlobalText[229]);
		break;
	}
}

void SEASON3B::CNewUIItem3DInputMsgBox::AddButtonBlank(int iAddLine)
{
	switch (m_dwMsgBoxType)
	{
	case MSGBOX_COMMON_TYPE_OK:
...
void SEASON3B::CNewUIItem3DInputMsgBox::RenderTexts()
{
	float x = GetPos().x + 95;
	float y = GetPos().y + 95;
	...
	for (; vi != m_MsgTextList.end(); vi++)
	{
		...
		//x = GetPos().x + MSGBOX_TEXT_LEFT_BLANK_3DITEM + ((GetSize().cx - MSGBOX_TEXT_LEFT_BLANK_3DITEM) / 2) - (TextExtentWidth / 2);
		g_pRenderText->RenderText((int)x, (int)y, (*vi)->strMsg.c_str(), 0, 0, RT3_WRITE_CENTER);
		y += (TextExtentHeight + 4);
	}
}

void SEASON3B::CNewUIItem3DInputMsgBox::RenderButtons()
{
	switch (m_dwMsgBoxType)
	{
	case MSGBOX_COMMON_TYPE_OK:
...
bool SEASON3B::CNewUIItem3DInputMsgBoxLayout::SetLayout()
{
	CNewUIItem3DInputMsgBox* pMsgBox = GetMsgBox();
	if (0 == pMsgBox)
		return false;
	...
	pMsgBox->SetInputBoxOption(UIOPTION_NUMBERONLY | UIOPTION_PAINTBACK);
	pMsgBox->AddMsg(GlobalText[421]);
	//pMsgBox->AddCallbackFunc(CNewUIItem3DInputMsgBoxLayout::ReturnDown, MSGBOX_EVENT_PRESSKEY_RETURN);
	//pMsgBox->AddCallbackFunc(CNewUIItem3DInputMsgBoxLayout::OkBtnDown, MSGBOX_EVENT_USER_COMMON_OK);
	//pMsgBox->AddCallbackFunc(CNewUIItem3DInputMsgBoxLayout::CancelBtnDown, MSGBOX_EVENT_USER_COMMON_CANCEL);

	pMsgBox->AddCallbackFunc(CNewUIItem3DInputMsgBoxLayout::ReturnDown, MSGBOX_EVENT_PRESSKEY_RETURN);
	pMsgBox->AddCallbackFunc(CNewUIItem3DInputMsgBoxLayout::OkBtnDown, MSGBOX_EVENT_USER_COMMON_OK);
	pMsgBox->AddCallbackFunc(CNewUIItem3DInputMsgBoxLayout::CancelBtnDown, MSGBOX_EVENT_USER_COMMON_CANCEL);

	pMsgBox->Set3DItem(g_pMyInventory->FindItemByKey(g_pMyInventory->GetStandbyItemKey()));
	return true;
}

CALLBACK_RESULT SEASON3B::CNewUIItem3DInputMsgBoxLayout::ReturnDown(CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	return ProcessOk(pOwner, xParam);
}

CALLBACK_RESULT SEASON3B::CNewUIItem3DInputMsgBoxLayout::OkBtnDown(CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	return ProcessOk(pOwner, xParam);
}

CALLBACK_RESULT SEASON3B::CNewUIItem3DInputMsgBoxLayout::CancelBtnDown(CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	PlayBuffer(SOUND_CLICK01);
	g_MessageBox->SendEvent(pOwner, MSGBOX_EVENT_DESTROY);
...
CALLBACK_RESULT SEASON3B::CNewUIItem3DInputMsgBoxLayout::ProcessOk(CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	PlayBuffer(SOUND_CLICK01);
	g_MessageBox->SendEvent(pOwner, MSGBOX_EVENT_DESTROY);
...
bool SEASON3B::CPersonalShopProItemValueMsgBoxLayout::SetLayout()
{
	CNewUIItem3DInputMsgBox* pMsgBox = GetMsgBox();
	if (0 == pMsgBox)
		return false;
	...
	pMsgBox->SetInputBoxOption(UIOPTION_NUMBERONLY | UIOPTION_PAINTBACK, 0);
	pMsgBox->SetInputBoxOption(UIOPTION_NUMBERONLY | UIOPTION_PAINTBACK, 1);
	pMsgBox->SetInputBoxOption(UIOPTION_NUMBERONLY | UIOPTION_PAINTBACK, 2);
	pMsgBox->SetInputBoxOption(UIOPTION_NUMBERONLY | UIOPTION_PAINTBACK, 3);

	pMsgBox->AddMsg(GlobalText[1129]);
	pMsgBox->AddCallbackFunc(CPersonalShopProItemValueMsgBoxLayout::ReturnDown, MSGBOX_EVENT_PRESSKEY_RETURN);
	pMsgBox->AddCallbackFunc(CPersonalShopProItemValueMsgBoxLayout::OkBtnDown, MSGBOX_EVENT_USER_COMMON_OK);
	pMsgBox->AddCallbackFunc(CPersonalShopProItemValueMsgBoxLayout::CancelBtnDown, MSGBOX_EVENT_USER_COMMON_CANCEL);

	return true;
}

CALLBACK_RESULT SEASON3B::CPersonalShopProItemValueMsgBoxLayout::ReturnDown(CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	return ProcessOk(pOwner, xParam);
}

CALLBACK_RESULT SEASON3B::CPersonalShopProItemValueMsgBoxLayout::OkBtnDown(CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	return ProcessOk(pOwner, xParam);
}

CALLBACK_RESULT SEASON3B::CPersonalShopProItemValueMsgBoxLayout::CancelBtnDown(CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	SEASON3B::CNewUIInventoryCtrl::BackupPickedItem();
	g_pMyShopInventory->SetInputValueTextBox(false);
	PlayBuffer(SOUND_CLICK01);
...
CALLBACK_RESULT SEASON3B::CPersonalShopProItemValueMsgBoxLayout::ProcessOk(CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	CNewUIItem3DInputMsgBox* pMsgBox = dynamic_cast<CNewUIItem3DInputMsgBox*>(pOwner);

	unicode::t_char strText[4][MAX_TEXT_LENGTH] = { 0, };

	pMsgBox->GetInputBoxText(strText[0], 0);
	pMsgBox->GetInputBoxText(strText[1], 1);
	pMsgBox->GetInputBoxText(strText[2], 2);
	pMsgBox->GetInputBoxText(strText[3], 3);

	if (unicode::_strlen(strText[0]) == 0
		&& unicode::_strlen(strText[1]) == 0
		&& unicode::_strlen(strText[2]) == 0
		&& unicode::_strlen(strText[3]) == 0
		)
	{
		return CALLBACK_CONTINUE;
	}

	int iInputZen = atoi(strText[0]);
	int iInputJoB = atoi(strText[1]);
	int iInputJoS = atoi(strText[2]);
	int iInputJoC = atoi(strText[3]);

	if (iInputZen == 0 && iInputJoB == 0 && iInputJoS == 0 && iInputJoC == 0)
	{
		return CALLBACK_CONTINUE;
	}
	...
	if (pPickedItem)
	{
		...
		if (InvenIndex != -1)
		{
			...
			BYTE byIndex = MAX_EQUIPMENT_INDEX + iSourceIndex;
			SendRequestSetSalePrice(byIndex, iInputZen, iInputJoB, iInputJoS, iInputJoC, pMsgBox->GetPersonalType());
			SendRequestEquipmentItem(REQUEST_EQUIPMENT_INVENTORY, MAX_EQUIPMENT + iSourceIndex, pItemObj, REQUEST_EQUIPMENT_MYSHOP, MAX_MY_INVENTORY_INDEX + iTargetIndex);
		}
		else if (pPickedItem->GetOwnerInventory() == NULL)
		{
		...
		AddPersonalItemPrice(MAX_MY_INVENTORY_INDEX + iTargetIndex, pMsgBox->GetPersonalType(), iInputZen, iInputJoB, iInputJoS, iInputJoC, g_IsPurchaseShop);
	}
	else
	{
		iSourceIndex = g_pMyShopInventory->GetSourceIndex();
		...
		BYTE byIndex = iSourceIndex;
		SendRequestSetSalePrice(byIndex, iInputZen, iInputJoB, iInputJoS, iInputJoC, pMsgBox->GetPersonalType());
		AddPersonalItemPrice(iSourceIndex, pMsgBox->GetPersonalType(), iInputZen, iInputJoB, iInputJoS, iInputJoC, g_IsPurchaseShop);
	}
