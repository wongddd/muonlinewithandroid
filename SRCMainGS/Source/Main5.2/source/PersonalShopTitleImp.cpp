...
bool CPersonalShopTitleImp::AddShopTitle(int key, CHARACTER* pPlayer, const std::string& title)
{
	...
	else
	{
		...
		{
			pPlayer->OpenStore(GlobalText[1104], pPlayer->ID, title.c_str());
		}
...
void CPersonalShopTitleImp::Draw()
{
	if (!m_listShopTitleDrawObj.empty())
	{
		...
		for (; mi != m_listShopTitleDrawObj.end(); ++mi)
		{
			...
			{
				CHARACTER* pSeletedPlayer = gmCharacters->GetCharacter(SelectedCharacter);

				if (pSeletedPlayer && (*mi).first == pSeletedPlayer)
				{
...
{
	float Width = GetWindowsX;
	float Height = GetWindowsY;
...
void CPersonalShopTitleImp::CalculateBooleanPos(IN CHARACTER* pPlayer, IN const SIZE& size, OUT POINT& pos)
{
	...
	Projection2(posTemp, &_ScreenX, &_ScreenY);
	//. pos : real position
	pos.x = (_ScreenX) - size.cx / 2;
	pos.y = (_ScreenY) - (36);
}
...
void CPersonalShopTitleImp::CShopTitleDrawObj::GetTopTitle(std::string& title)
{
}

void CPersonalShopTitleImp::CShopTitleDrawObj::GetbottomTitle(std::string& title)
{
}
...
void CPersonalShopTitleImp::CShopTitleDrawObj::Draw(int iPkLevel)
{
	...
	POINT RenderPos = { m_pos.x, m_pos.y };
	SIZE RenderBoxSize = { m_size.cx, m_size.cy };
	SIZE RenderIconSize = { m_icon.cx, m_icon.cy };
	int iLineHeight = FontHeight;

	g_pRenderText->SetFont(g_hFontBold);
	g_pRenderText->SetBgColor(iIconBkColor);
	g_pRenderText->SetTextColor(iIconTextColor);
	g_pRenderText->RenderTextClipped(RenderPos.x, RenderPos.y, GlobalText[1104], RenderIconSize.cx, iLineHeight);

	g_pRenderText->SetBgColor(iNameBkColor);
	g_pRenderText->SetTextColor(iNameTextColor);
	g_pRenderText->RenderTextClipped(RenderPos.x + RenderIconSize.cx, RenderPos.y, m_fullname.c_str(), RenderBoxSize.cx - RenderIconSize.cx, iLineHeight);
	...
	if (!m_bottomTitle.empty())
	{
		...
		g_pRenderText->RenderTextClipped(RenderPos.x, RenderPos.y, m_topTitle.c_str(), RenderBoxSize.cx, iLineHeight);
		RenderPos.y += iLineHeight;
		g_pRenderText->RenderTextClipped(RenderPos.x, RenderPos.y, m_bottomTitle.c_str(), RenderBoxSize.cx, iLineHeight);
	}
...
void CPersonalShopTitleImp::CShopTitleDrawObj::SeparateShopTitle(IN const std::string& title, OUT std::string& topTitle, OUT std::string& bottomTitle)
{
	char pszTopTitle[MAX_SHOPTITLE] = { '\0' };
	char pszBottonTitle[MAX_SHOPTITLE] = { '\0' };
	CutText(title.c_str(), pszBottonTitle, pszTopTitle, MAX_SHOPTITLE);
