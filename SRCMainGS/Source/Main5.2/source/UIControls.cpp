...
int CutStr(const char* pszSrcText, char* pTextOut, const int iTargetPixelWidth, const int iMaxOutLine, const int iOutStrLength, const int iFirstLineTab /* = 0 */)
{
	...
	int iConversionType = CHINESE_CP;
#else
...
CUITextInputBox* g_pFocusedAndroidTextBox = nullptr;

extern "C" void Android_DeliverIMEText(const char* utf8) {
	if (g_pFocusedAndroidTextBox) {
		g_pFocusedAndroidTextBox->AppendIMEText(utf8);
	}
}

extern "C" void Android_TextBox_GetText(void* pBox, char* out, int maxLen) {
	if (pBox && out && maxLen > 0) {
		CUITextInputBox* pTextBox = static_cast<CUITextInputBox*>(pBox);
		pTextBox->GetText(out, maxLen);
	}
}

void __attribute__((weak)) Android_ShowKeyboard(bool show) {
	// Default no-op; overridden in MuBridge.cpp with JNI callback
}
#endif
...
BOOL CUIButton::DoMouseAction()
{
	...
	{
		MouseOnWindow = true;
		if (MouseLButtonPush && !g_dwCurrentPressedButtonID)
		{
			m_bMouseState = TRUE;
...
BOOL CUIGuildListBox::RenderDataLine(int iLineNumber)
{
	...
	if (m_TextListIter->m_Server != 0xFF/* && m_TextListIter->m_Number != 0*/)
	{
...
void CUISocketListBox::RenderInterface()
{
	...
	{
		SEASON3B::RenderGoldRect(m_iPos_x - 1, m_iPos_y - m_iHeight - 1, m_iWidth + 1, m_iHeight + 2, 2);
	}
...
bool CUIRenderText::Create(int iRenderTextType, HDC hDC)
{
	...
	{
#ifdef ANDROID
		m_pRenderText = new CUIRenderTextFT;
#else
...
HFONT CUIRenderText::GetFont()
{
	if (m_pRenderText)
		return static_cast<HFONT>(GetCurrentObject(GetFontDC(), OBJ_FONT));
	return NULL;
}
...
DWORD CUIRenderText::GenerateSinWaveColor(float frc1, float frc2, float frc3, BYTE phs1, BYTE phs2, BYTE phs3, BYTE center, BYTE width, BYTE step)
{
	BYTE red = (std::sin(frc1 * step + phs1) * width + center);
	BYTE grn = (std::sin(frc2 * step + phs2) * width + center);
	BYTE blu = (std::sin(frc3 * step + phs3) * width + center);
	BYTE alp = 0xff;

	return ARGB(alp, red, grn, blu);
}

void CUIRenderText::RenderWave(int posX, int posY, LPCSTR pszText, int width, int height, int iSort, SIZE* lpTextSize, int Size, bool fScale)
{
	char filter[10];
	char bufer[255];

	DWORD ColorBack1 = this->GetBgColor();
	DWORD ColorBack2 = this->GetTextColor();

	HFONT FontBack = this->GetFont();

	this->SetFont(g_hFontBold);

	//this->SetTextColor(0xFFFFFFFF);
	if (fScale)
		this->RenderText(posX, posY, pszText, width, height, iSort, lpTextSize);
	else
		this->RenderTextClipped(posX, posY, pszText, width, height, iSort, lpTextSize);

	this->SetBgColor(0);

	for (BYTE i = 0; i < Size; i++)
	{
		DWORD Color = GenerateSinWaveColor(0.5f, 0.5f, 0.5f, 0, 2, 4, 128, 127, (i + GetTickCount() / 100 % 255));
		...
		if (fScale)
			this->RenderText(posX, posY, bufer, width, height, iSort, NULL);
		else
			this->RenderTextClipped(posX, posY, bufer, width, height, iSort, NULL);
	}

	this->SetFont(FontBack);

	this->SetBgColor(ColorBack1);
	this->SetTextColor(ColorBack2);
}

void CUIRenderText::RenderText(int iPos_x, int iPos_y, const char* pszText, int iBoxWidth, int iBoxHeight, int iSort, OUT SIZE* lpTextSize, bool black_out)
{
	if (m_pRenderText)
	{
		m_pRenderText->RenderText(iPos_x, iPos_y, pszText, iBoxWidth, iBoxHeight, iSort, lpTextSize, black_out);
	}
}

void CUIRenderText::RenderFont(int iPos_x, int iPos_y, const char* pszText, int iBoxWidth, int iBoxHeight, int iSort, OUT SIZE* lpTextSize)
{
	if (m_pRenderText)
	{
		m_pRenderText->RenderFont(iPos_x, iPos_y, pszText, iBoxWidth, iBoxHeight, iSort, lpTextSize);
	}
}

void CUIRenderText::RenderTextClipped(float iPos_x, float iPos_y, const char* pszText, int iBoxWidth, int iBoxHeight, int iSort, OUT SIZE* lpTextSize)
{
	if (m_pRenderText)
	{
		m_pRenderText->RenderTextClipped(iPos_x, iPos_y, pszText, iBoxWidth, iBoxHeight, iSort, lpTextSize);
	}
}
...
HDC CUIRenderTextOriginal::GetFontDC() const
{
	return m_hFontDC;
}

BYTE* CUIRenderTextOriginal::GetFontBuffer() const
{
	return m_pFontBuffer;
}

DWORD CUIRenderTextOriginal::GetTextColor() const
{
	return m_dwTextColor;
}

DWORD CUIRenderTextOriginal::GetBgColor() const
{
...
void CUIRenderTextOriginal::UploadText(int sx, int sy, int Width, int Height, bool black_out)
{
	...
	if (Width > 0 && Height > 0 && sx + Width > 0 && sy + Height > 0)
	{
		...
		if (black_out)
		{
			...
			RenderBitmap(BITMAP_FONT, (float)sx + 1.f, (float)sy + 1.f, (float)Width, (float)Height, TextureU, TextureV, TextureUWidth, TextureVHeight, false, false);
...
void CUIRenderTextOriginal::RenderText(int iPos_x, int iPos_y, const unicode::t_char* pszText, int iBoxWidth, int iBoxHeight, int iSort, OUT SIZE* lpTextSize, bool black_out)
{
	if (pszText == NULL || (pszText[0] == '\0' && iBoxWidth == 0))
		return;

	if (strlen(pszText) <= 0 && iBoxWidth == 0)
		return;
	...
	if (RealBoxSize.cy == 0)
		...
		RealBoxPos.y += ((RealBoxSize.cy - RealTextSize.cy) / 2);
	...
	else if (iSort == RT3_WRITE_CENTER_TO_TOP)
	{
		if (RealRenderingSize.cx > RealBoxSize.cx)
		{
			iClipMove = (RealRenderingSize.cx - RealBoxSize.cx) / 2;
	...
	if (m_dwBackColor != 0)
	{
		...
		float rx = (RealBoxPos.x / g_fScreenRate_x) - 1.f;
		float wx = (RealBoxSize.cx / g_fScreenRate_x) + 2.0;
		RenderColor(rx, RealBoxPos.y / g_fScreenRate_y, wx, RealBoxSize.cy / g_fScreenRate_y);
		EndRenderColor();
	}
	...
	for (int i = 0; i < iNumberOfSections; i++)
	{
		...
		UploadText(RealBoxPos.x + LIMIT_WIDTH * i + iTab, RealBoxPos.y, RealSectionLine.cx, RealSectionLine.cy, black_out);
	}
...
void CUIRenderTextOriginal::RenderFont(int iPos_x, int iPos_y, const unicode::t_char* pszText, int iBoxWidth, int iBoxHeight, int iSort, OUT SIZE* lpTextSize)
{
	if (pszText == NULL || (pszText[0] == '\0' && iBoxWidth == 0))
		return;
	if (strlen(pszText) <= 0 && iBoxWidth == 0)
	...
	if (m_dwBackColor != 0)
	{
		EnableAlphaTest();
		glColor4ub(GetRed(m_dwBackColor), GetGreen(m_dwBackColor), GetBlue(m_dwBackColor), GetAlpha(m_dwBackColor));
		RenderColor(RealBoxPos.x / g_fScreenRate_x, iPos_y, RealBoxSize.cx / g_fScreenRate_x, RealBoxSize.cy / g_fScreenRate_y);
		EndRenderColor();
	}
...
void CUIRenderTextOriginal::RenderTextClipped(float iPos_x, float iPos_y, const unicode::t_char* pszText, int iBoxWidth, int iBoxHeight, int iSort, OUT SIZE* lpTextSize)
{
	if (pszText == NULL || (pszText[0] == '\0' && iBoxWidth == 0))
		return;
	if (strlen(pszText) <= 0 && iBoxWidth == 0)
	...
	MU_POINTF RealBoxPos = { (float)iPos_x, (float)iPos_y };
	SIZEF RealBoxSize = { (float)iBoxWidth, (float)iBoxHeight };
	SIZEF RealRenderingSize = { (long)RealTextSize.cx, (long)RealTextSize.cy };
	...
	if (RealBoxSize.cy == 0)
		...
		RealBoxPos.y += ((RealBoxSize.cy - RealTextSize.cy) / 2.f);
	...
	if (m_dwBackColor != 0)
	{
		EnableAlphaTest();
		glColor4ub(GetRed(m_dwBackColor), GetGreen(m_dwBackColor), GetBlue(m_dwBackColor), GetAlpha(m_dwBackColor));
		RenderColor(RealBoxPos.x - 2, RealBoxPos.y, RealBoxSize.cx + 4, RealBoxSize.cy, 0.0, 0, false);
		EndRenderColor();
	}
	...
	for (int i = 0; i < iNumberOfSections; i++)
	{
		...
		WriteText(LIMIT_WIDTH * i * 3 + iClipMove, RealSectionLine.cx, RealSectionLine.cy);
		UploadText(RealBoxPos.x + LIMIT_WIDTH * i + iTab, RealBoxPos.y, RealSectionLine.cx, RealSectionLine.cy, false);
	}
...
#ifdef ANDROID
	if (g_pFocusedAndroidTextBox == this)
	{
		g_pFocusedAndroidTextBox = nullptr;
	...
	case WM_CHAR:
		...
		default:
		{
			...
			else if (pTextInputBox->CheckOption(UIOPTION_SERIALNUMBER))
			{
				...
				&& wParam != VK_BACK && !((Char >= 33 && Char <= 126)
					|| (Char >= 0xE01 && Char <= 0xE2E) || (Char >= 0xE30 && Char <= 0xE3A)))
			{
				return 0;
			}
			//else
			//{
			//	if (wParam != VK_BACK
			//		&& !(wParam == 0x03 || wParam == 0x18)
			//		&& Char != 0x16
			//		&& !pTextInputBox->CheckLimiteRange(Char))
			//	{
			//		return 0;
			//	}
			//}
...
{
	if (m_bIsReady == FALSE || m_hEditWnd == NULL)
		return;
...
void CUITextInputBox::GetText(char* pszText, int iGetLength)
{
	if (pszText == NULL)
		return;
	...
	if (m_hEditWnd == NULL)
	{
		// On Android, read from internal buffer (no Win32 Edit control)
		std::wstring wstrText = m_wszTextBuf;
		std::string strText;
		g_pMultiLanguage->ConvertWideCharToStr(strText, wstrText.c_str(), g_pMultiLanguage->GetCodePage());
		strncpy_s(pszText, iGetLength, strText.c_str(), _TRUNCATE);
		return;
	}
#endif

	wchar_t pwszText[256] = {0};

	GetWindowTextW(m_hEditWnd, pwszText, iGetLength);

	for (int i = 0; i < iGetLength; i++)
	{
		pwszText[i] = g_pMultiLanguage->ConvertFulltoHalfWidthChar(pwszText[i]);
	}
...
void CUITextInputBox::GetText(wchar_t* pwszText, int iGetLenght)
{
	...
	if (m_hEditWnd == NULL)
	{
		wcsncpy(pwszText, m_wszTextBuf, iGetLenght);
		return;
	}
#endif
...
int CUITextInputBox::GetAsInt()
{
	char lpszCheck[256] = { 0 }; // Asegurar inicializaci�n

	this->GetText(lpszCheck, sizeof(lpszCheck)); // Obtener el texto en char

	return atoi(lpszCheck);
}

void CUITextInputBox::SetByteLimite(BYTE iLimit)
{
	m_iLimitByte = iLimit;
}

bool CUITextInputBox::CheckLimiteRange(wchar_t Char)
{
	if (m_iLimitByte == 0xFF) // Sin l�mite de bytes
		return true;

	char pszCheck[256] = { 0 };

	this->GetText(pszCheck, sizeof(pszCheck));


	std::string strText;
	std::wstring pwszText = L"" + g_pMultiLanguage->ConvertFulltoHalfWidthChar(Char);

#ifdef CHINESE_LANGUAGE
	g_pMultiLanguage->ConvertWideCharToStr(strText, pwszText.c_str(), CHINESE_CP);
#else
	g_pMultiLanguage->ConvertWideCharToStr(strText, pwszText.c_str(), g_pMultiLanguage->GetCodePage());
#endif

	std::string lpszCheck = pszCheck;

	return (lpszCheck.length() + strText.length() < m_iLimitByte);
}

void CUITextInputBox::SetPlaceHolder(const char* pszText)
{
	textPlaceholder = pszText;
}

bool CUITextInputBox::bmi_header_work()
{
	if ((int)(m_iWidth * g_fScreenRate_x) != m_iRealWindowX || (int)(m_iHeight * g_fScreenRate_y) != m_iRealWindowY)
	{
		UpdateBmiHeader(m_iWidth, m_iHeight);
	}
	return true;
}

bool CUITextInputBox::UpdateBmiHeader(int iWidth, int iHeight)
{
	m_iWidth = 0;
	m_iHeight = 0;
...
void CUITextInputBox::SetText(const char* pszText)
{
	...
	if (m_hEditWnd == NULL)
	{
		...
		if (wstrText.length() > 255)
			wcsncpy(m_wszTextBuf, wstrText.c_str(), 255);
		else
			wcscpy(m_wszTextBuf, wstrText.c_str());
		m_wszTextBuf[255] = 0;
		return;
	}
#endif
...
{
	if ((iWidth == 0 || iHeight == 0) && iWidth == m_iWidth && iHeight == m_iHeight)
		return;

	m_iWidth = iWidth;
	m_iHeight = iHeight;

	m_iRealWindowX = (m_iWidth * g_fScreenRate_x);
	m_iRealWindowY = (m_iHeight * g_fScreenRate_y);
...
void CUITextInputBox::Init(HWND hWnd, int iWidth, int iHeight, int iMaxLength, BOOL bIsPassword)
{
	...
	if (m_hEditWnd == NULL)
	{
		m_hEditWnd = CreateWindowW(L"edit", NULL, WS_CHILD | WS_VISIBLE | dwOptionFlag, m_iRealWindowPos_x, m_iRealWindowPos_y, iWidth * g_fScreenRate_x, iHeight * g_fScreenRate_y, m_hParentWnd, (HMENU)ID_UICEDIT, gwinhandle->GetInstance(), NULL);
	}
	else
	{
		SetWindowPos(m_hEditWnd, 0, 0, 0, iWidth * g_fScreenRate_x, iHeight * g_fScreenRate_y, SWP_NOMOVE | SWP_NOZORDER);
...
void CUITextInputBox::GiveFocus(BOOL SelectText)
{
	...
	{
#ifdef ANDROID
		g_pFocusedAndroidTextBox = this;
		g_dwKeyFocusUIID = GetUIID();
		Android_ShowKeyboard(true);
#endif
		return;
	}

	if (g_iChatInputType == 1 && GetFocus() == gwinhandle->GethWnd() && !CheckOption(UIOPTION_SERIALNUMBER) && !CheckOption(UIOPTION_NUMBERONLY))
	{
		SetFocus(m_hEditWnd);
		RestoreIMEStatus();
	}
		...
		for (int x = 0; x < iWidth; ++x)
		{
			...
			if (*(m_pFontBuffer + SrcIndex) == 255)
			{
				if (bIsCaretTime && PtInRect(&rcCaret, ptProcessing))
					*((unsigned int*)(buff + DstIndex)) = m_dwBackColor;
				else
					*((unsigned int*)(buff + DstIndex)) = m_dwTextColor;
			}
			...
			{
				if (bIsCaretTime && PtInRect(&rcCaret, ptProcessing))
					*((unsigned int*)(buff + DstIndex)) = _ARGB(255, 200, 200, 200);
				else
					*((unsigned int*)(buff + DstIndex)) = m_dwBackColor;
			}
			else
			{
				if (bIsCaretTime && PtInRect(&rcCaret, ptProcessing))
					...
					*((unsigned int*)(buff + DstIndex)) = m_dwSelectBackColor;
			}
			SrcIndex += 3;
			DstIndex += 4;
		}
...
void CUITextInputBox::Render()
{
	...
	if (m_hEditWnd == NULL)
	{
		// Android rendering path — uses Android Canvas-rendered text texture
		...
		if (CheckOption(UIOPTION_PAINTBACK))
		{
			...
			RenderColor(m_iPos_x - 1, m_iPos_y - 1, m_iWidth, m_iHeight);
			EndRenderColor();
		}

		// Read text from internal buffer
		char szText[256] = { 0 };
		GetText(szText, 255);

		// Render the Java-side text texture (if any text has been committed)
		extern void Android_RenderTextTexture(float x, float y, float maxWidth, float maxHeight);

		if (szText[0] != '\0')
		{
			float padX = 4.0f;
			float padY = 4.0f;
			float textMaxW = (float)m_iWidth - padX * 2;
			float textMaxH = (float)m_iHeight - padY * 2;
			Android_RenderTextTexture((float)m_iPos_x + padX, (float)m_iPos_y + padY, textMaxW, textMaxH);
		}
	...
	if (m_bUseMultiLine == FALSE)
	{
		...
		if (TextCheck[0] == '\0' && TextCheckUTF16[0] == L'\0' && !textPlaceholder.empty() && !HaveFocus())
		{
			g_pRenderText->SetBgColor(0);
			g_pRenderText->SetTextColor(150, 150, 150, 220);
			HFONT _hBack = g_pRenderText->GetFont();
			g_pRenderText->SetFont(g_hFont);
			g_pRenderText->RenderText(m_iPos_x, m_iPos_y, textPlaceholder.c_str(), m_iWidth, m_iHeight, RT3_SORT_LEFT);
			g_pRenderText->SetFont(_hBack);
		}
		...
		if (SceneFlag == MAIN_SCENE && RealWndSize.cy > RealTextLine.cy)
		{
			RealWndPos.y += ((RealWndSize.cy - RealTextLine.cy) / 2.f);
		}
...
#ifdef ANDROID
void CUITextInputBox::AppendIMEText(const char* utf8)
{
	if (utf8 == NULL || utf8[0] == '\0')
		return;

	std::wstring wstrText;
	g_pMultiLanguage->ConvertCharToWideStr(wstrText, utf8);

	size_t curLen = wcslen(m_wszTextBuf);
	size_t addLen = wstrText.length();
	if (curLen + addLen > 254)
		addLen = 254 - curLen;
	if (addLen > 0)
	{
		wcsncpy(m_wszTextBuf + curLen, wstrText.c_str(), addLen);
		m_wszTextBuf[curLen + addLen] = 0;
	}
}
#endif
...
CUISlideHelp::CUISlideHelp()
{
	...
	m_pszSlideText = NULL;
	m_dwCurrentSecond = 0;
	memset(m_szSlideTextA, 0, sizeof(m_szSlideTextA));
	memset(m_szSlideTextB, 0, sizeof(m_szSlideTextB));
}
...
void CUISlideHelp::Init(BOOL bBold, BOOL bBlink)
{
	...
	m_fMovePosition = gwinhandle->GetScreenX();
	m_fMoveAccel = 1.0f;
	m_fMoveSpeed = 0;
	m_iCutLength = 0;
	m_iCutSize = 0;
...
void CUISlideHelp::Render(BOOL bForceFadeOut)
{
	...
	if (m_bBlink == TRUE && g_iNoticeInverse % 10 < 5)
		byAlpha /= 2;
...
void CUISlideHelp::ComputeSpeed()
{
	...
	m_fMovePosition -= timefac(m_fMoveSpeed);
}
...
void CSlideHelpMgr::Init()
{
	...
	SetTimer(gwinhandle->GethWnd(), SLIDEHELP_TIMER, m_iCreateDelay * 1000, 0);
	CreateSlideText();
}
...
void CSlideHelpMgr::OpenSlideTextFile(const char* szFileName)
{
	...
	SLIDEHELP* SlideHelp = new SLIDEHELP;
...
BOOL CUINewGuildMemberListBox::RenderDataLine(int iLineNumber)
{
	...
	if (iCharacterLevel == 0)
		g_pRenderText->RenderText(iPos_x + 45, iPos_y, GlobalText[1300], 70, 0, RT3_SORT_CENTER);
	else if (iCharacterLevel == 1)
		g_pRenderText->RenderText(iPos_x + 45, iPos_y, GlobalText[1301], 70, 0, RT3_SORT_CENTER);
	else if (iCharacterLevel == 2)
		g_pRenderText->RenderText(iPos_x + 45, iPos_y, GlobalText[1302], 70, 0, RT3_SORT_CENTER);
...
BOOL CUIUnmixgemList::RenderDataLine(int iLineNumber)
{
	...
	ITEM* pItem = g_pMyInventory->FindItem(m_TextListIter->m_iInvenIdx);
...
void CUIBCGuildListBox::AddText(const char* szGuildName, BYTE byJoinSide, BYTE byGuildInvolved, int iGuildScore)
{
	if (szGuildName == NULL || szGuildName[0] == '\0') return;
...
void CUIQuestContentsListBox::RenderCoveredInterface()
{
	...
	if (QUEST_REQUEST_ITEM == m_TextListIter->m_dwType)
	{
		...
		g_pNewItemTooltip->RenderItemTooltip(nX, nY, m_TextListIter->m_pItem, false, 0);
	}
}
