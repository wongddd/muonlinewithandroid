...
void CNewChaosCastleSystem::SetMatchGameCommand(const LPPRECEIVE_MATCH_GAME_STATE data)
{
	...
	case 8:
		if (m_byCurrCastleLevel == 0)
		{
			...
			SetActionObject(gMapManager->currentMap, 1, 40, 1);
	...
	if (m_byMatchType > 0 && gMapManager->InChaosCastle())
	{
		...
		case 8:
			if (m_iMatchTime > 0)
			{
				...
				g_pChaosCastleTime->SetKillMonsterStatue(m_iKillMonster, m_iMaxKillMonster);if (m_iKillMonster <= 46 && m_iKillMonster > 40)
				{
