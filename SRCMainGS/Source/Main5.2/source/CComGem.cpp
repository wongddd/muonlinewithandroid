...
{ITEM* pItem = NULL;
	...
	{
		int InvenIndex = g_pMyInventory->FindInvenIndex(i);

		if (InvenIndex == g_pMyInventory->GetInvenEnableCnt())
			break;

		pNewInventoryCtrl = g_pMyInventory->GetInventoryCtrl(InvenIndex);

		int SrcIndex = g_pMyInventory->SlotCntReceiveInvenIndex(i, 64);

		pItem = pNewInventoryCtrl->GetItem(SrcIndex);
		...
		if (isCompiledGem(pItem))
		{
			...
			p.first = g_pMyInventory->GetInvenSlotIndex(pItem->x, pItem->y, InvenIndex);
			p.second = Level;
...
{int NumEmptyInv = 0;
	...
	{
		SEASON3B::CNewUIInventoryCtrl* pNewInventoryCtrl = g_pMyInventory->GetInventoryCtrl(i);
		NumEmptyInv += pNewInventoryCtrl->GetEmptySlotCount();
	}
	return NumEmptyInv;
	return 0;}
...
int COMGEM::Check_Jewel(int _nJewel, int _nType, bool _bModel)
{
	...
	if (bNon)
	{
		if (_nJewel == ITEM_POTION + 13)
			return eBLESS;
		if (_nJewel == ITEM_POTION + 14)
			return eSOUL;
		if (_nJewel == ITEM_POTION + 16)
			return eLIFE;
		if (_nJewel == ITEM_POTION + 22)
			return eCREATE;
		if (_nJewel == ITEM_POTION + 31)
			return ePROTECT;
		if (_nJewel == ITEM_POTION + 41)
			return eGEMSTONE;
		if (_nJewel == ITEM_POTION + 42)
			return eHARMONY;
		if (_nJewel == ITEM_WING + 15)
			return eCHAOS;
		if (_nJewel == ITEM_POTION + 43)
			return eLOW;
		if (_nJewel == ITEM_POTION + 44)
			return eUPPER;
	}

	if (bCom)
	{
		if (_nJewel == ITEM_WING + 30)
			return eBLESS_C;
		if (_nJewel == ITEM_WING + 31)
			return eSOUL_C;
		if (_nJewel == ITEM_WING + 136)
			return eLIFE_C;
		if (_nJewel == ITEM_WING + 137)
			return eCREATE_C;
		if (_nJewel == ITEM_WING + 138)
			return ePROTECT_C;
		if (_nJewel == ITEM_WING + 139)
			return eGEMSTONE_C;
		if (_nJewel == ITEM_WING + 140)
			return eHARMONY_C;
		if (_nJewel == ITEM_WING + 141)
			return eCHAOS_C;
		if (_nJewel == ITEM_WING + 142)
			return eLOW_C;
		if (_nJewel == ITEM_WING + 143)
			return eUPPER_C;
	}
