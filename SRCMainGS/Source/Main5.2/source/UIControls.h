...
enum REQUEST_REWARD_CLASSIFY : int;
...
public:
	...
	virtual void RenderText(int iPos_x, int iPos_y, const char* pszText, int iBoxWidth = 0, int iBoxHeight = 0, int iSort = RT3_SORT_LEFT, OUT SIZE* lpTextSize = NULL, bool black_out = false) = 0;
	virtual void RenderFont(int iPos_x, int iPos_y, const char* pszText, int iBoxWidth = 0, int iBoxHeight = 0, int iSort = RT3_SORT_LEFT, OUT SIZE* lpTextSize = NULL) = 0;
	virtual void RenderTextClipped(float iPos_x, float iPos_y, const char* pszText, int iBoxWidth = 0, int iBoxHeight = 0, int iSort = RT3_SORT_LEFT, OUT SIZE* lpTextSize = NULL) = 0;
};
...
public:
	...
	void RenderText(int iPos_x, int iPos_y, const unicode::t_char* pszText, int iBoxWidth = 0, int iBoxHeight = 0, int iSort = RT3_SORT_LEFT, OUT SIZE* lpTextSize = NULL, bool black_out = false);
	void RenderFont(int iPos_x, int iPos_y, const unicode::t_char* pszText, int iBoxWidth = 0, int iBoxHeight = 0, int iSort = RT3_SORT_LEFT, OUT SIZE* lpTextSize = NULL);
	void RenderTextClipped(float iPos_x, float iPos_y, const unicode::t_char* pszText, int iBoxWidth = 0, int iBoxHeight = 0, int iSort = RT3_SORT_LEFT, OUT SIZE* lpTextSize = NULL);
protected:
	void WriteText(int iOffset, int iWidth, int iHeight);
	void UploadText(int sx,int sy,int Width,int Height, bool black_out = false);
};
...
public:
	...
	DWORD GenerateSinWaveColor(float frc1, float frc2, float frc3, BYTE phs1, BYTE phs2, BYTE phs3, BYTE center, BYTE width, BYTE step);
	void RenderWave(int posX, int posY, LPCSTR pszText, int width, int height, int iSort, SIZE* lpTextSize, int Size, bool fScale = true);
	void RenderText(int iPos_x, int iPos_y, const char* pszText, int iBoxWidth = 0, int iBoxHeight = 0, int iSort = RT3_SORT_LEFT, OUT SIZE* lpTextSize = NULL, bool black_out = false);
	void RenderFont(int iPos_x, int iPos_y, const char* pszText, int iBoxWidth = 0, int iBoxHeight = 0, int iSort = RT3_SORT_LEFT, OUT SIZE* lpTextSize = NULL);
	void RenderTextClipped(float iPos_x, float iPos_y, const char* pszText, int iBoxWidth = 0, int iBoxHeight = 0, int iSort = RT3_SORT_LEFT, OUT SIZE* lpTextSize = NULL);
...
public:
	...
	int GetAsInt();
	void SetByteLimite(BYTE iLimit);
	bool CheckLimiteRange(wchar_t Char);
	void SetPlaceHolder(const char* pszText);
	bool bmi_header_work();
	bool UpdateBmiHeader(int iWidth, int iHeight);
...
#ifdef ANDROID
	void AppendIMEText(const char* utf8);
#endif
...
protected:
	...
	DWORD m_iLimitByte;
#ifdef ANDROID
	wchar_t m_wszTextBuf[256];
	wchar_t m_wszRenderBuf[256];
#endif
};
...
{
public:
	CUIChatInputBox():m_bHistoryMode(0),
		m_szTempText("\0")
