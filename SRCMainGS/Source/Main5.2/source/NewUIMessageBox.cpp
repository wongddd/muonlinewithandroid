...
bool SEASON3B::CNewUIMessageBoxBase::Create(int x, int y, int width, int height, float fPriority/* = 3.f*/, bool fixMID)
{
	if (fixMID == true)
	{
		x = PositionX_The_Mid(width);
		y = PositionY_The_Mid(height);
	}
...
void SEASON3B::CNewUIMessageBoxBase::RenderMsgBackColor(bool _bRender)
{
	...
	float _fWidth = gwinhandle->GetScreenX(), _fHeight = gwinhandle->GetScreenY();
...
SEASON3B::CNewUIMessageBoxMng::CNewUIMessageBoxMng() : m_pNewUIMng(NULL), m_pMsgBoxFactory(NULL), m_EventState(EVENT_NONE)
{
...
bool SEASON3B::CNewUIMessageBoxMng::UpdateKeyEvent()
{
	...
	{
		SendEvent(pCurMsgBox, MSGBOX_EVENT_PRESSKEY_Y);
	}
...
#if MAIN_UPDATE > 603
	LoadBitmap("Interface\\HUD_TEST\\newui_message_line.dds", BITMAP_msgbox_line, GL_LINEAR);
	//LoadBitmap("Interface\\HUD_TEST\\newui_bt_clearness_illusion.dds", BITMAP_msgbox_btn, GL_LINEAR, GL_CLAMP_TO_EDGE, true, false);
	//LoadBitmap("Interface\\HUD_TEST\\ingame_bt01.dds", BITMAP_msgbox_btn, GL_LINEAR, GL_CLAMP_TO_EDGE, true, false);
	LoadBitmap("Interface\\HUD_TEST\\newui_button_ok.dds", BITMAP_msgbox_btn_ok, GL_LINEAR, GL_CLAMP_TO_EDGE, true, false);
	LoadBitmap("Interface\\HUD_TEST\\newui_button_cancel.dds", BITMAP_msgbox_btn_cancel, GL_LINEAR, GL_CLAMP_TO_EDGE, true, false);
	LoadBitmap("Interface\\HUD_TEST\\newui_button_close.dds", BITMAP_msgbox_btn_close, GL_LINEAR, GL_CLAMP_TO_EDGE, true, false);
	LoadBitmap("Interface\\HUD_TEST\\newui_btn_empty.dds", BITMAP_msgbox_btn_empty, GL_LINEAR, GL_CLAMP_TO_EDGE, true, false);
	LoadBitmap("Interface\\HUD_TEST\\newui_btn_empty_small.dds", BITMAP_msgbox_btn_empty_small, GL_LINEAR, GL_CLAMP_TO_EDGE, true, false);
	LoadBitmap("Interface\\HUD_TEST\\newui_btn_empty_big.dds", BITMAP_msgbox_btn_empty_big, GL_LINEAR, GL_CLAMP_TO_EDGE, true, false);
	LoadBitmap("Interface\\HUD_TEST\\newui_btn_empty_very_small.dds", BITMAP_msgbox_btn_empty_very_small, GL_LINEAR, GL_CLAMP_TO_EDGE, true, false);
	LoadBitmap("Interface\\HUD_TEST\\msgbox_system_menu.dds", BITMAP_msgbox_back_frame, GL_LINEAR, GL_CLAMP_TO_EDGE, true, false);
	LoadBitmap("Interface\\HUD_TEST\\newui_msgbox_back.dds", BITMAP_window_box_2048, GL_LINEAR, GL_CLAMP_TO_EDGE, true, false);
#endif
}
