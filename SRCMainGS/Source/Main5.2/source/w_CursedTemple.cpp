...
bool CursedTemple::IsPartyMember(DWORD selectcharacterindex)
{
	...
	CHARACTER* c = gmCharacters->GetCharacter(selectcharacterindex);
...
bool CursedTemple::RenderObjectVisual(OBJECT* o, BMD* b)
{
	...
	case WD_45CURSEDTEMPLE_LV6:
	{
		...
		case 54:
		{
			...
			CreateParticleSync(BITMAP_EFFECT, position, o->Angle, o->Light);
			CreateParticleSync(BITMAP_EFFECT, position, o->Angle, o->Light, 3);
		}
		break;
		case 70:
		{
			if (rand() % 3 == 0)
			{
				...
				CreateParticleSync(BITMAP_CLOUD, o->Position, o->Angle, Light, 15, o->Scale, o);
			}
		}
		return true;
		case 71:
		...
		case 74:
		{
			if (rand() % 2 == 0)
			{
				...
				CreateParticleSync(BITMAP_CLOUD, o->Position, o->Angle, Light, 16, o->Scale, o);
			}
		}
		return true;
		case 75:
		{
			if (rand() % 35 == 5)
			{
				...
				CreateParticleSync(BITMAP_GHOST_CLOUD1, o->Position, o->Angle, Light, 0, o->Scale, o);
			}
		}
		return true;
		case 76:
		{
			...
			for (int i = 0; i < 1; ++i)
			{
				CreateParticleSync(BITMAP_TORCH_FIRE, vPos, o->Angle, Light, 0, o->Scale, o);
			}
			VectorCopy(o->Position, vPos);
			vPos[2] += 20.f;
		...
		case 78:
		{
			...
			if (iTime >= iRand + (rand() % 5) && iTime < iRand + (rand() % 10 + 5))
			{
				...
				for (int i = 0; i < 4; ++i)
				{
					CreateEffectSync(MODEL_FALL_STONE_EFFECT, o->Position, o->Angle, Light);
				}
				CreateEffectSync(MODEL_FALL_STONE_EFFECT, o->Position, o->Angle, Light, 1);
				Vector(0.7f, 0.7f, 0.8f, Light);
				vec3_t vPos;
				VectorCopy(o->Position, vPos);
				vPos[0] += (float)(rand() % 80 - 40);
				vPos[1] += (float)(rand() % 80 - 40);
				CreateParticleSync(BITMAP_WATERFALL_3 + (rand() % 2), vPos, o->Angle, Light, 2);
				Vector(0.9f, 0.0f, 0.0f, Light);
				CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, Light, 0, 1.5f);
			}
		}
		return true;
		case 79:
		{
			if (rand() % 1 == 0)
			{
				for (int i = 0; i < 5; ++i)
				{
					...
					CreateParticleSync(BITMAP_CLOUD, o->Position, o->Angle, Light, 17, o->Scale, o);
				}
			}
		}
		return true;
...
bool CursedTemple::RenderMonsterVisual(CHARACTER* c, OBJECT* o, BMD* b)
{
	...
	case MODEL_CURSEDTEMPLE_ILLUSION_NPC:
	{
		...
		for (int j = 0; j < 3; ++j)
		{
			...
			CreateParticleSync(BITMAP_CLUD64, vWorldPos, o->Angle, Light, 0, 0.7f);
		}
	}
	break;
	...
	case MODEL_MONSTER01 + 125:
	{
		...
		if (o->CurrentAction == MONSTER01_DIE)
		{
			...
			{
				b->TransformPosition(o->BoneTransform[boneindex[i]], vRelativePos, vWorldPos, true);
				CreateParticleSync(BITMAP_SMOKE + 3, vWorldPos, o->Angle, Light, 3, 0.5f);
			}
		}
	}
	return true;
	case MODEL_MONSTER01 + 124:
	{
		...
		if (o->CurrentAction == MONSTER01_ATTACK1 || o->CurrentAction == MONSTER01_ATTACK2)
		{
			...
			CreateParticleSync(BITMAP_CLUD64, vWorldPos, o->Angle, Light, 4, 0.8f);
			CreateParticleSync(BITMAP_SPARK + 1, vWorldPos, o->Angle, Light, 15, 0.7f + (fLuminosity * 0.05f));
	...
	case MODEL_MONSTER01 + 123:
	{
		...
		if (o->CurrentAction == MONSTER01_ATTACK1 || o->CurrentAction == MONSTER01_ATTACK2)
		{
			Vector(0.6f, 0.0f, 0.0f, Light);
			b->TransformPosition(o->BoneTransform[17], vRelativePos, vWorldPos, true);
			CreateEffectSync(MODEL_FENRIR_THUNDER, vWorldPos, o->Angle, Light, 2, o);
			CreateEffectSync(MODEL_FENRIR_THUNDER, vWorldPos, o->Angle, Light, 2, o);
			CreateEffectSync(MODEL_FENRIR_THUNDER, vWorldPos, o->Angle, Light, 2, o);
			CreateParticleSync(BITMAP_CLUD64, vWorldPos, o->Angle, Light, 4, 1.f);
			CreateSprite(BITMAP_LIGHT, vWorldPos, 3.f, Light, o, 0.f);
	...
	case MODEL_CURSEDTEMPLE_STATUE:
		...
		else
		{
			...
			Vector(0.f, 0.f, 0.f, vRelativePos);
			b->TransformPosition(o->BoneTransform[13], vRelativePos, vWorldPos, true);
			CreateParticleSync(BITMAP_FLARE + 1, vWorldPos, o->Angle, Light, 0, 0.15f);
			vWorldPos[2] += 30;
			CreateParticleSync(BITMAP_LIGHT, vWorldPos, o->Angle, Light, 1, 8.0f);

			vWorldPos[2] += 160;
			Vector(0.2f, 0.1f, 0.0f, Light);
			b->TransformPosition(o->BoneTransform[14], vRelativePos, vWorldPos, true);
			CreateParticleSync(BITMAP_LIGHT, vWorldPos, o->Angle, Light, 1, 4.0f);
		}
		return true;
...
void CursedTemple::ReceiveCursedTempleInfo(BYTE* ReceiveBuffer)
{
	...
	if (data->btUserIndex != 0xffff)
	{
		...
		{
			CHARACTER* c = gmCharacters->GetCharacter(holyitemcharacterindex);
			OBJECT* o = &c->Object;
