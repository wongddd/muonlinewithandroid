...
void ChangeChaosCastleUnit(CHARACTER* c)
{
	...
	ClearChaosCastleHelper(c);DWORD t_dwUIID = g_pWindowMgr->GetAddFriendWindow();
	if (t_dwUIID != 0)
	{
...
{
	if (gMapManager->InChaosCastle() == false || rand() % 8)
		return;
	...
	if (InArea)
	{
		...
		Position[2] = Hero->Object.Position[2];
		CreateParticleSync(BITMAP_SMOKE + 4, Position, Angle, Light, 0, 1.5f);
