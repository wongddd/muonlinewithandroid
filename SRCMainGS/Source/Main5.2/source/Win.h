...
public:
	...
	BYTE GetBgAlpha() { return m_psprBg ? m_psprBg->GetAlpha() : 0; }
	void SetBgAlpha(BYTE byAlpha) { if (m_psprBg) m_psprBg->SetAlpha(byAlpha); }
	void SetBgColor(BYTE byRed, BYTE byGreen, BYTE byBlue)
	{ if (m_psprBg) m_psprBg->SetColor(byRed, byGreen, byBlue); }
