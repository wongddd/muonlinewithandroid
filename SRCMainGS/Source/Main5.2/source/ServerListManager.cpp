...
void CServerListManager::InsertServerGroup(int iConnectIndex, int iServerPercent, char* iServerName)
{
	...
	InsertServer(pServerGroup, iConnectIndex, iServerPercent, iServerName);
...
void CServerListManager::InsertServer(CServerGroup* pServerGroup, int iConnectIndex, int iServerPercent, char* iServerName)
{
	...
	if (!gmProtect->ServerNameDefault && iServerName && iServerName[0] != '\0')
	{
		sprintf(pServerInfo->m_bName, "%s-%d", iServerName, pServerInfo->m_iIndex);
	}
	else
	{
		...
		case 1:
			...
			break;
		case 2:
			sprintf(pServerInfo->m_bName, "%s-%d(Gold PVP) %s", pServerGroup->m_szName, pServerInfo->m_iIndex, GlobalText[iTextIndex]);
			break;
		case 3:
			sprintf(pServerInfo->m_bName, "%s-%d(Gold) %s", pServerGroup->m_szName, pServerInfo->m_iIndex, GlobalText[iTextIndex]);
			break;
		}
	}
