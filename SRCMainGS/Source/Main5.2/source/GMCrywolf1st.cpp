...
{
	if (gMapManager->currentMap == WD_34CRYWOLF_1ST && m_OccupationState == CRYWOLF_OCCUPATION_STATE_PEACE)
		return true;
...
void M34CryWolf1st::CheckCryWolf1stMVP(BYTE btOccupationState, BYTE btCrywolfState)
{
	...
	case CRYWOLF_OCCUPATION_STATE_OCCUPIED:
		...
		break;
	case CRYWOLF_OCCUPATION_STATE_WAR:
		sprintf(FileName, "Data\\%s\\EncTerrain%d.att", WorldName, (gMapManager->currentMap + 1) * 10 + 2);
		break;
	}
...
void M34CryWolf1st::ChangeBackGroundMusic(int iWorld)
{
	if (iWorld == WD_34CRYWOLF_1ST)
	{
...
{
	return (gMapManager->currentMap == WD_34CRYWOLF_1ST) ? true : false;
}
...
bool M34CryWolf1st::RenderCryWolf1stObjectMesh(OBJECT* o, BMD* b, int ExtraMon)
{
	...
	case 71:
	{
		...
		{
			Vector(1.f, 1.f, 1.f, Light);
			CreateParticleSync(BITMAP_TRUE_FIRE, o->Position, o->Angle, Light, 5, o->Scale);
		}
	}
	return true;
	case 72:
	{
		if (rand() % 10 == 0)
		{
			...
			Luminosity = (float)(rand() % 4 + 3) * 0.1f;
			Vector(Luminosity, Luminosity * 0.6f, Luminosity * 0.2f, Light);
			CreateParticleSync(BITMAP_SMOKE, Position, o->Angle, Light, 33, o->Scale);
		}
	}
	return true;
	case 73:
	...
	case 74:
	{
		...
		{
			Vector(1.f, 1.f, 1.f, Light);
			CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, Light, 21, o->Scale);
		}
	}
	return true;
	case 77:
	{
		if (rand() % 6 == 0)
		{
			...
			CreateParticleSync(BITMAP_CLOUD, o->Position, o->Angle, Light, 9, o->Scale, o);
		}
	}
	return true;
	case 78:
	...
	case MODEL_MONSTER01 + 89:
	{
		...
		if (o->CurrentAction == MONSTER01_ATTACK2)
		{
			...
			if (o->AnimationFrame >= 7.5f && o->AnimationFrame < (7.5f + fActionSpeed))
			{
				...
				Vector(1.f, 1.f, 1.f, Light);
				CreateEffectSync(BITMAP_CRATER, EndPos, o->Angle, o->Light, 2);
	...
	case MODEL_MONSTER01 + 93:
	{
		...
		for (int i = 0; i < gmClientModels->GetModel(o->Type)->NumMeshs; i++)
		{
			if (i == 2)
			{
				...
				b->EndRender();
				b->BeginRender(o->Alpha);
				gmClientModels->GetModel(o->Type)->StreamMesh = i;
			}
			else
				o->BlendMesh = -1;
